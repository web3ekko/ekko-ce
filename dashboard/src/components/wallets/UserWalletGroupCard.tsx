...
export function UserWalletGroupCard({
  ...
  const [routing, setRouting] = useState<NotificationRoutingChoice>(
    group.notification_routing,
  );
  const [autoSubscribe, setAutoSubscribe] = useState<boolean>(
  ...
  const recomputeHasChanges = (next: {
    routing: NotificationRoutingChoice;
  ...
  const canManageWallets = Boolean(
    onAddWallets || onRemoveWallet || onBulkImport,
  );
  ...
  const handleRemoveWallet = async (walletKey: string) => {
    ...
    if (!confirm(`Remove ${walletKey} from "${group.wallet_group_name}"?`))
      return;
    setIsWalletSaving(true);
    try {
        ...
        <Group gap="sm">
          ...
          <Stack gap={2}>
            ...
            <Text size="xs" c="#64748B">
              {group.wallet_keys.length} wallets managed by{" "}
              {group.provider_name}
            </Text>
          </Stack>
        </Group>
        ...
        <Group gap="xs">
          ...
          {onDisconnect && (
            <ActionIcon
              ...
              onClick={async () => {
                if (!confirm(`Disconnect from "${group.wallet_group_name}"?`))
                  return;
                try {
          ...
          <ActionIcon
            variant="subtle"
            aria-label="Toggle provider wallet group details"
            onClick={toggle}
          >
            {expanded ? (
        ...
        <Stack gap="md" mt="md">
          ...
          <Group grow>
            ...
            <Switch
              ...
              onChange={(e) =>
                handleAutoSubscribeChange(e.currentTarget.checked)
              }
            />
          </Group>
