...
export function WebhookChannelCard({
  channel,
  onToggle,
  onDelete,
  onTest,
  stats,
}: WebhookChannelCardProps) {
  const [isToggling, setIsToggling] = useState(false);
  const [isTesting, setIsTesting] = useState(false);
  const [isDeleting, setIsDeleting] = useState(false);
  const [healthMetrics, setHealthMetrics] =
    useState<WebhookHealthMetrics | null>(null);
          ...
          <Card withBorder padding="sm" radius="sm" bg="gray.0">
            <Stack gap="sm">
              ...
              <Group justify="space-around">
                ...
                <div style={{ textAlign: "center" }}>
                  ...
                  <Text size="lg" fw={700} c="blue">
                    {healthMetrics.avg_response_time_ms}ms
                  </Text>
                </div>
              </Group>
              ...
              {healthMetrics.consecutive_failures > 0 && (
                ...
                >
                  <Text size="xs">
                    {healthMetrics.consecutive_failures} consecutive{" "}
                    {healthMetrics.consecutive_failures === 1
                      ? "failure"
                      : "failures"}
                    {healthMetrics.consecutive_failures >= 3 &&
              ...
              <Group justify="space-between">
                {healthMetrics.last_success_at && (
                  <div>
                    ...
                    <Text size="xs">
                      {formatTimestamp(healthMetrics.last_success_at)}
                    </Text>
                  </div>
                )}
              </Group>
