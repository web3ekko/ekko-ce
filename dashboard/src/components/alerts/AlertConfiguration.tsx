...
export function AlertConfiguration({
      ...
      case "string":
        if (param.validation?.options) {
          return (
            <Select
              {...commonProps}
              data={param.validation.options.map((opt) => ({
                value: opt,
                label: opt,
              }))}
            />
  ...
  const getParametersByCategory = () => {
    ...
    const remaining = template.parameters.filter(
      (p) => !categorized.includes(p),
    );
      ...
      <Stack gap="xl">
        ...
        {isValid && (
          <Alert color="green" variant="light" icon={<IconCheck size="1rem" />}>
            <Text size="sm">Configuration is valid and ready to proceed.</Text>
          </Alert>
        )}
