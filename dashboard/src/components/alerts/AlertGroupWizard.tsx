...
type AlertGroupAlertType =
  "wallet" | "network" | "protocol" | "token" | "contract" | "nft";
...
const ALERT_TYPES: Array<{
...
}> = [
  {
    value: "wallet",
    label: "Wallet",
    description: "Targets wallet keys and wallet groups",
  },
  {
    value: "network",
    label: "Network",
    description: "Targets network keys (e.g. ETH:mainnet) and network groups",
  },
  {
    value: "protocol",
    label: "Protocol",
    description:
      "Targets protocol keys (e.g. ETH:mainnet:aave-v3) and protocol groups",
  },
  {
    value: "token",
    label: "Token",
    description: "Targets token contract keys and token groups",
  },
  {
    value: "contract",
    label: "Contract",
    description: "Targets contract keys and contract groups",
  },
  {
    value: "nft",
    label: "NFT",
    description: "Targets NFT collections (and later token_id) via NFT groups",
  },
];
...
function buildTargetingVariableIds(
  alertType: AlertGroupAlertType,
): Set<string> {
  const base = new Set([
    "network",
    "networks",
  ...
  if (alertType === "protocol") {
    ["protocol_key", "protocol", "protocol_name", "protocol_id"].forEach((id) =>
      base.add(id),
    );
    return base;
  }
...
function getTemplateTargetAlertType(
...
export function AlertGroupWizard({
  opened,
  onClose,
  onSuccess,
}: AlertGroupWizardProps) {
  ...
  const loadTemplates = async () => {
    ...
    try {
      ...
      const response = await httpClient.get<any>(
        API_ENDPOINTS.ALERT_TEMPLATES.LIST,
      );
      const payload = response.data;
      const results: AlertTemplate[] = Array.isArray(payload)
        ? payload
        : payload?.results || [];
        ...
        <Group gap="sm">
          ...
          <div>
            ...
            <Text size="xs" c="dimmed">
              Build a collection of alerts to share or subscribe
            </Text>
          </div>
        </Group>
      }
        ...
        >
          <Stack gap="md" mt="xl">
            ...
            <Select
              label="Alert Type"
              description={
                ALERT_TYPES.find((t) => t.value === form.values.alert_type)
                  ?.description
              }
        ...
        >
          <Stack gap="md" mt="xl">
            <Alert
              ...
              variant="light"
            >
              Select alert templates to include in your group. Users who
              subscribe will receive alerts from these templates.
            </Alert>
            ...
            <Alert
              ...
              variant="light"
            >
              Alert groups require templates to share the same template type and
              required settings so subscribers only fill one configuration form.
            </Alert>
              ...
              <>
                {(() => {
                  const eligibleTemplates = templates.filter(
                    (template) =>
                      getTemplateTargetAlertType(template) ===
                      form.values.alert_type,
                  );

                  const baselineId = form.values.selectedTemplates[0];
                  const baselineTemplate = baselineId
                    ? templates.find((t) => t.id === baselineId)
                    : undefined;
                  const baselineTemplateType =
                    baselineTemplate?.template_type?.toLowerCase() || null;
                  const baselineRequired = baselineTemplate
                    ? getTemplateRequiredParamIds(baselineTemplate)
                    : null;

                  return (
                    <>
                      <Text size="sm" c="dimmed">
                        {form.values.selectedTemplates.length} selected •{" "}
                      ...
                      <SimpleGrid cols={{ base: 1, md: 2 }} spacing="md">
                        {eligibleTemplates.map((template) => {
                          const isSelected =
                            form.values.selectedTemplates.includes(template.id);

                          let disabledReason: string | null = null;
                          if (
                            baselineTemplate &&
                            template.id !== baselineTemplate.id
                          ) {
                            if (
                              baselineTemplateType &&
                              ...
                              baselineRequired &&
                              !setsEqual(
                                getTemplateRequiredParamIds(template),
                                baselineRequired,
                              )
                            ) {
                      ...
                      {eligibleTemplates.length === 0 && (
                        <Center h={150}>
                          <Text c="dimmed">
                            No templates available for this alert type
                          </Text>
                        </Center>
                      )}
                    </>
        ...
        >
          <Stack gap="xl" mt="xl">
              ...
              style={{
                border:
                  form.values.visibility === "public"
                    ...
                    : "1px solid #E6E9EE",
                background:
                  form.values.visibility === "public" ? "#EFF6FF" : "#FFFFFF",
                cursor: "pointer",
              }}
            ...
            >
              <Group>
                ...
                <Stack gap={2} style={{ flex: 1 }}>
                  ...
                  <Text size="sm" c="dimmed">
                    Anyone can discover and subscribe to this group. Great for
                    sharing valuable alerts with the community.
                  </Text>
                </Stack>
                <Checkbox
              ...
              style={{
                border:
                  form.values.visibility === "private"
                    ...
                    : "1px solid #E6E9EE",
                background:
                  form.values.visibility === "private" ? "#EFF6FF" : "#FFFFFF",
                cursor: "pointer",
              }}
            ...
            >
              <Group>
                ...
                <Stack gap={2} style={{ flex: 1 }}>
                  ...
                  <Text size="sm" c="dimmed">
                    Only you can see this group. Perfect for personal
                    organization or testing before making public.
                  </Text>
                </Stack>
                <Checkbox
            ...
            <Alert
              ...
              variant="light"
            >
              You can change visibility settings at any time from the group
              settings.
            </Alert>
          </Stack>
        </Stepper.Step>
            ...
            <Paper p="md" radius="md" withBorder>
              <Stack gap="md">
                <Group justify="space-between" align="flex-start">
                  ...
                  <Badge
                    color={
                      CATEGORIES.find((c) => c.value === form.values.category)
                        ? "blue"
                        : "gray"
                    }
                  >
                  ...
                  <div>
                    ...
                    <Group gap={6} mt={4}>
                      <Badge color="blue" variant="light">
                        {ALERT_TYPES.find(
                          (t) => t.value === form.values.alert_type,
                        )?.label || form.values.alert_type}
                      </Badge>
                    </Group>
                  </div>
                  <div>
