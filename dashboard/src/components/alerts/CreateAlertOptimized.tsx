...
export function CreateAlertOptimized({
  ...
  const [triggerType, setTriggerType] = useState<
    "event_driven" | "periodic" | "one_time"
  >("event_driven");
  const [triggerTypeTouched, setTriggerTypeTouched] = useState(false);
  const [frequencyPreset, setFrequencyPreset] = useState<
    "realtime" | "5min" | "15min" | "1hour" | "daily"
  >("realtime");
  ...
  const [nlpContextOverrides, setNlpContextOverrides] = useState<
    Record<string, unknown>
  >({});
  ...
  const scheduleNlpSubmit = useCallback(
    (
      ...
      const cooldownRemaining = Math.max(
        0,
        lastSubmitAtRef.current + NLP_SUBMIT_COOLDOWN_MS - now,
      );
  ...
  const rerunTemplateCompiler = useCallback(
  ...
  useEffect(() => {
    ...
    let isActive = true;
    const load = async () => {
      const resp = await alertsApiService.getTemplateLatest(
        initialTemplateRef.templateId,
      );
      if (!isActive) return;
      ...
      const variables = Array.isArray((templateSpec as any)?.variables)
        ? ((templateSpec as any).variables as any[])
        : [];
      ...
      setProposedSpec({
        ...
        human_preview: {
          summary: String(
            (templateSpec as any)?.name || "Marketplace template",
          ),
          segments: [],
        },
      });
      ...
      const scopeNetworks = Array.isArray(
        (templateSpec as any)?.scope?.networks,
      )
        ? ((templateSpec as any).scope.networks as string[])
        : [];
  ...
  useEffect(() => {
    ...
    const baseDelay = contextChangeRef.current
      ? NLP_CONTEXT_DEBOUNCE_MS
      : NLP_DEBOUNCE_MS;
    contextChangeRef.current = false;
  ...
  useEffect(() => {
    if (proposedSpec?.required_user_inputs?.supported_trigger_types?.length) {
      const supported =
        proposedSpec.required_user_inputs.supported_trigger_types;
  ...
  useEffect(() => {
    ...
    if (
      frequencyPreset === "5min" ||
      frequencyPreset === "15min" ||
      frequencyPreset === "1hour" ||
  ...
  useEffect(() => {
    const loadGroups = async () => {
      try {
        const walletGroupsResponse = await groupsApiService.getGroupsByType(
          GroupType.WALLET,
        );
        ...
        const accountsGroup =
          groups.find(
            (group) =>
              (group.settings as { system_key?: string })?.system_key ===
              "accounts",
          ) || null;
        const orderedGroups = accountsGroup
          ? [
              accountsGroup,
              ...groups.filter((group) => group.id !== accountsGroup.id),
            ]
          : groups;
  ...
  useEffect(() => {
    ...
    deferredNaturalLanguage,
    handleLocalUnderstanding,
    isUsingMarketplaceTemplate,
  ]);
  ...
  const requiredVariableIds = useMemo(() => {
    ...
    const required =
      proposedSpec?.required_user_inputs?.required_variables || [];
  ...
  const missingVariables = useMemo(() => {
    ...
    return variablesToRender.filter((variable) => {
      ...
      if (!id) return false;
      return (
        requiredVariableIds.has(id.toLowerCase()) &&
        !isValueProvided(variableValues[id])
      );
    });
  }, [variablesToRender, requiredVariableIds, variableValues]);

  const targetsRequired =
    proposedSpec?.required_user_inputs?.targets_required !== false;
  ...
  const preferredNetwork =
    typeof nlpContextOverrides.preferred_network === "string"
  ...
  const canTestAlert =
    ...
    (proposedSpecConfidencePct === null || proposedSpecConfidencePct >= 60);
  ...
  const targetKeys = useMemo(() => {
    ...
    return targetKeysInput
      ...
      .filter(Boolean)
      .map((value) =>
        value.includes(":") ? value : `${defaultNetworkKey}:${value}`,
      );
  ...
  const buildTriggerConfig = () => {
    if (triggerType === "event_driven") {
      const networks = templateScopeNetworks.length
        ? templateScopeNetworks
        : [defaultNetworkKey];
      return { networks };
    }
  ...
  useEffect(() => {
    ...
    const mode = String(
      (proposedSpec as any)?.template?.trigger?.evaluation_mode || "",
    ).toLowerCase();
    ...
    else if (mode === "hybrid" && triggerType !== "event_driven")
  ...
  useEffect(() => {
    ...
    const cadence = Number(
      (proposedSpec as any)?.template?.trigger?.cron_cadence_seconds,
    );
  ...
  const handleSaveTemplate = async () => {
    ...
    if (response.success) {
      ...
      response.code === "marketplace_template_exists" &&
      ...
      notifications.show({
        ...
        message:
          "Use the existing marketplace template instead of creating a duplicate.",
        color: "blue",
      });
    } else {
  ...
  const ensureSavedTemplateForCreate =
    async (): Promise<AlertTemplateSaveResponse | null> => {
      if (savedTemplate?.template_id && savedTemplate.template_version)
        return savedTemplate;
      if (isUsingMarketplaceTemplate) return savedTemplate;

      if (!jobId || nlpStatus !== "completed") {
        setCreateError(
          "Please wait for analysis to complete before creating an alert.",
        );
        notifications.show({
      ...
      const message =
        response.message ||
        "Failed to save the template needed to create this alert.";
  ...
  const handleTestAlert = async () => {
    ...
    if (proposedSpec.template && !proposedSpec.compiled_executable) {
      const chain =
        normalizeChainKey(understanding.chains?.[0] || "ethereum") ||
        "ethereum";
      const previewConfig = {
      ...
      await runInlinePreview(
        proposedSpec.template as Record<string, unknown>,
        previewConfig,
        "wallet",
      );
      return;
  ...
  const handleAdjustThreshold = () => {
    ...
    notifications.show({
      ...
      message:
        "Modify the threshold in your description to change trigger sensitivity",
      color: "blue",
    });
  };
  ...
  return (
    <Stack gap="xl" style={{ maxWidth: 840, margin: "0 auto", width: "100%" }}>
        ...
        <Group gap="xs" wrap="wrap" align="center">
          ...
          {understanding.eventType && (
            <AlertEventBadge
              ...
              subEvent={understanding.subEvent}
              chain={understanding.chains?.[0]}
            />
          )}
      ...
      <NaturalLanguageInputEnhanced
        ...
        isProcessing={
          !isUsingMarketplaceTemplate &&
          (nlpStatus === "connecting" || nlpStatus === "processing")
        }
      />
          ...
          <Stack gap="md">
            <Group justify="space-between" wrap="wrap" align="center">
              ...
              <Group gap="xs" wrap="wrap">
                ...
                <Switch
                  ...
                  onChange={(e) =>
                    setPublishToMarketplace(e.currentTarget.checked)
                  }
                />
              </Group>
            </Group>
            ...
            <Group justify="space-between">
              <Text size="sm" c="dimmed">
                Save a reusable template (optional). Creating the alert will
                auto-save if needed.
              </Text>
              <Button
                variant="light"
                ...
                <Stack gap={6}>
                  ...
                  {networkMissingInfo && (
                    <Select
                      ...
                      data={(
                        networkMissingInfo.options as Array<{
                          id: string;
                          label?: string;
                        }>
                      ).map((opt) => ({
                      ...
                      onChange={(next) => {
                        ...
                        ).find((opt) => opt.id === next);
                        const patch = selected?.context_patch;
                        if (patch && typeof patch === "object") {
                          rerunTemplateCompiler(
            ...
            {Array.isArray(proposedSpec?.assumptions) &&
              proposedSpec.assumptions.length > 0 && (
        ...
        <Stack gap="md">
          <Group justify="space-between">
            ...
            <ActionIcon
              ...
              aria-label="Toggle alert instance settings"
            >
              {advancedOpened ? (
              ...
              <div>
                ...
                <SegmentedControl
                  ...
                  onChange={(value) => {
                    ...
                    setTriggerType(
                      value as "event_driven" | "periodic" | "one_time",
                    );
                  }}
                />
                ...
                {triggerType === "periodic" && (
                  <Select
                    ...
                    onChange={(value) => {
                      ...
                      setFrequencyPreset(
                        (value || "5min") as typeof frequencyPreset,
                      );
                    }}
                  />
                )}
                ...
                <div>
                  ...
                  <Stack gap="sm">
                    {variablesToRender.map((variable) => {
                      ...
                      const isRequired = requiredVariableIds.has(
                        id.toLowerCase(),
                      );
                      ...
                      if (type === "enum" && variable.validation?.options) {
                        const options = (
                          variable.validation.options as Array<{
                            value: string;
                            label?: string;
                          }>
                        ).map((opt) => ({
                ...
                <div>
                  ...
                  <Stack gap={4}>
                    <Text size="xs" c="dimmed">
                      Model:{" "}
                      {String(
                        (proposedSpec.pipeline_metadata as any)?.model ||
                          "unknown",
                      )}
                    </Text>
                    <Text size="xs" c="dimmed">
                      Latency:{" "}
                      {String(
                        (proposedSpec.pipeline_metadata as any)?.latency_ms ??
                          "n/a",
                      )}{" "}
                      ms
                    </Text>
                    {(proposedSpec.pipeline_metadata as any)
                      ?.stage_timings_ms &&
                      typeof (proposedSpec.pipeline_metadata as any)
...
const extractChains = (input: string): string[] => {
  ...
  if (lowerInput.includes("sol") || lowerInput.includes("solana"))
    chains.push("solana");
  if (lowerInput.includes("avax") || lowerInput.includes("avalanche"))
    chains.push("avalanche");
  if (lowerInput.includes("matic") || lowerInput.includes("polygon"))
    chains.push("polygon");
  if (lowerInput.includes("arb") || lowerInput.includes("arbitrum"))
    chains.push("arbitrum");
  if (lowerInput.includes("optimism") || lowerInput.includes("op "))
    chains.push("optimism");
  if (lowerInput.includes("base")) chains.push("base");
  if (lowerInput.includes("bsc") || lowerInput.includes("bnb"))
    chains.push("bsc");
...
const extractThresholds = (input: string): ThresholdValue[] => {
  ...
  matches.forEach((match) => {
    ...
    if (
      unit.toLowerCase().includes("eth") ||
      unit.toLowerCase().includes("btc")
    ) {
...
const calculateConfidence = (
  ...
  if (understandingState.eventType && understandingState.eventType !== "custom")
    score += 30;
