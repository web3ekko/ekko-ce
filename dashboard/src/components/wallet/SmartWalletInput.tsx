...
export function SmartWalletInput({
  ...
  const validateFormat = useCallback(
    (addr: string, chain: ChainType): boolean => {
  ...
  const validateOnChain = useCallback(
    async (addr: string, chain: ChainType): Promise<ValidationResult> => {
      // Simulate API call
    ...
    <Stack spacing="xs">
      <Group position="apart" align="flex-end" noWrap>
        ...
        <TextInput
          ...
          disabled={disabled}
          error={
            validationResult.status === "invalid" && validationResult.message
          }
          ...
          styles={{
            ...
            input: {
              borderColor: getStatusColor()
                ? `var(--mantine-color-${getStatusColor()}-6)`
                : undefined,
            },
          }}
        />
