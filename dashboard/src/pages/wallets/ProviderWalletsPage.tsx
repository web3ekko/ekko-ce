...
export function ProviderWalletsPage() {
  ...
  const [importMergeMode, setImportMergeMode] = useState<"append" | "replace">(
    "append",
  );
  ...
  const [formRouting, setFormRouting] =
    useState<NotificationRoutingChoice>("callback_only");
  ...
  const load = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to load provider wallets",
      );
    } finally {
      setIsLoading(false);
    }
    ...
    const loadOptions = async () => {
      try {
        const [walletsResp, webhooksResp] = await Promise.all([
          ...
          notificationsApiService.getChannels({
  ...
  const handleUpdate = async (
    groupId: string,
    updates: UpdateUserWalletGroupRequest,
  ) => {
    try {
      const updated = await groupsApiService.updateUserWalletGroup(
        groupId,
        updates,
      );
  ...
  const handleAddWallets = async (groupId: string, walletKeys: string[]) => {
    try {
      await groupsApiService.addUserWalletGroupWallets(groupId, {
  ...
  const handleRemoveWallet = async (groupId: string, walletKey: string) => {
    try {
      await groupsApiService.removeUserWalletGroupWallets(groupId, {
  ...
  const handleImport = async () => {
    ...
    setIsSaving(true);
    try {
      const response: UserWalletGroupImportResponse =
        await groupsApiService.importUserWalletGroupWallets(importTargetId, {
          format: importFormat,
      ...
      notifications.show({
        title: "Error",
        message:
          err instanceof Error ? err.message : "Failed to import wallets",
        color: "red",
        icon: <IconAlertCircle size={16} />,
      });
    } finally {
    ...
    try {
      ...
      notifications.show({
        ...
        message:
          err instanceof Error
            ? err.message
            : "Failed to create provider wallet group",
        color: "red",
        icon: <IconAlertCircle size={16} />,
      });
    } finally {
  ...
  const selectedCallbackUrl = webhooks.find((hook) => hook.id === formCallback)
        ...
        <Group justify="space-between" align="flex-start">
          <div>
            ...
            <Text c="dimmed" size="sm" maw={760}>
              Wallets managed by a provider (e.g., exchange, wallet app). These
              do not appear in your Accounts. You control how notifications are
              delivered for each provider group.
            </Text>
          </div>
          <Button
            variant="light"
          ...
          <Card withBorder padding="xl" radius="md">
            <Stack gap="sm" align="center">
              ...
              <Text c="dimmed" size="sm" ta="center" maw={520}>
                When a provider connects wallets on your behalf, they will
                appear here with routing controls.
              </Text>
              <Button
                leftSection={<IconPlus size={16} />}
                onClick={() => {
      ...
      >
        <Stack>
          ...
          <Select
            ...
            placeholder={
              webhooks.length ? "Select a webhook" : "No webhook channels found"
            }
        ...
        <Stack>
          <Group grow>
            <Select
              ...
              value={importFormat}
              onChange={(value) =>
                setImportFormat(value === "json" ? "json" : "csv")
              }
            />
            <Select
              label="Merge Mode"
              ...
              onChange={(value) =>
                setImportMergeMode(value === "replace" ? "replace" : "append")
              }
            />
          </Group>
          <Textarea
