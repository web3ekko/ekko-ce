...
export function DashboardHomeExecutive() {
  ...
  const [
    ...
    { open: openAddWalletModal, close: closeAddWalletModal },
  ] = useDisclosure(false);
  const [
    inviteModalOpened,
  ...
  const [isNotificationStatsLoading, setIsNotificationStatsLoading] =
    useState(true);
  const { stats: dashboardStats, isLoading: isStatsLoading } =
    useDashboardStats();
    ...
    () => [
      {
        ...
        value: isNotificationStatsLoading
          ? "Loading"
      ...
      },
      {
        ...
        value: isStatsLoading
          ? "Loading"
          : (dashboardStats?.alerts.active ?? activeAlerts),
        variant: "info",
      },
      {
        label: "System Status",
        ...
        variant:
          isConnected && systemStatus.status === "operational"
            ? "success"
            : "warning",
      },
    ],
  ...
  const checklist = useMemo(() => {
    ...
    return [
      ...
      {
        label: "Set a notification channel",
        done: hasChannels,
    ...
    accounts.length,
    dashboardStats?.alerts.total,
    dashboardStats?.wallets.total,
    notificationTotals.channels,
  ]);
  ...
  useEffect(() => {
    ...
    if (showWelcome === "true") {
      notifications.show({
        ...
        message:
          "Your enhanced monitoring dashboard is now active. All critical alerts and portfolio updates will appear here.",
        color: "teal",
              ...
              <Button
                ...
                onClick={() =>
                  notifications.show({
                    ...
                    message: "Executive summary sent to email",
                    color: "teal",
                  })
                }
              >
            ...
            {quickActions.map((action) => (
              ...
              >
                <Group gap={6} wrap="nowrap">
                  ...
                  <Text fw={600} size="xs" c="#0F172A" lineClamp={1}>
                    {action.title}
                  </Text>
                </Group>
              </Card>
                ...
                {healthStats.map((stat) => (
                  <Group key={stat.label} gap={6} align="center">
                    {stat.variant === "warning" && (
                      <IconAlertTriangle size={14} color="#FB923C" />
                    )}
                    ...
                    <Group gap="md">
                      <div>
                        ...
                        <Text fw={700} size="sm" c="#0F172A">
                          {isStatsLoading
                            ? "—"
                            : (dashboardStats?.activity.executions_24h ?? 0)}
                        </Text>
                      </div>
                      <div>
                        <Text size="xs" c="dimmed">
                        ...
                        <Text fw={700} size="sm" c="#0F172A">
                          {isStatsLoading
                            ? "—"
                            : (dashboardStats?.activity.triggered_24h ?? 0)}
                        </Text>
                      </div>
                    </Group>
                  </Group>
