...
function getTargetKeyPlaceholder(alertType: string): string {
  ...
  if (normalized === "nft")
    return "e.g. ETH:mainnet:0xcollection (or …:token_id later)";
...
export function AlertGroupsPage() {
  ...
  const [wizardOpened, { open: openWizard, close: closeWizard }] =
    useDisclosure(false);
  const [
    subscribeModalOpened,
    { open: openSubscribeModal, close: closeSubscribeModal },
  ] = useDisclosure(false);
  ...
  const [selectedTargetGroup, setSelectedTargetGroup] = useState<string | null>(
    null,
  );
  ...
  const [accountsWalletKeys, setAccountsWalletKeys] = useState<
    Array<{ value: string; label: string }>
  >([]);
  const [templatesResponse, setTemplatesResponse] =
    useState<AlertGroupTemplatesResponse | null>(null);
  ...
  const isGroupSubscribed = (groupId: string) =>
    subscribedGroupIds.has(groupId);
  ...
  const handleSubscribe = async (groupId: string) => {
    ...
    if (group) {
      ...
      try {
        const groupAlertType =
          typeof group.settings?.alert_type === "string"
            ? (group.settings.alert_type as string)
            : "wallet";
        const targetGroupType = alertTypeToTargetGroupType(groupAlertType);

        const [targetGroupsResponse, accountsGroup, templates] =
          await Promise.all([
            groupsApiService.getGroupsByType(targetGroupType),
            groupAlertType.toLowerCase() === "wallet"
              ? groupsApiService.getAccountsGroup()
        ...
        if (!templates.templates.length) {
          notifications.show({
            title: "No templates",
            message:
              "This alert group has no templates yet. Ask the creator to add templates before subscribing.",
            color: "yellow",
        ...
        setSelectedTargetGroup(
          groupAlertType.toLowerCase() === "wallet" ? accountsId || null : null,
        );
        ...
        setAccountsWalletKeys(
          groupAlertType.toLowerCase() === "wallet" ? accountKeys : [],
        );
        ...
        const targetingIds = buildTargetingVariableIds(
          normalizeAlertType(templates.alert_type),
        );
  ...
  const handleConfirmSubscription = async () => {
    ...
    try {
      await createSubscription({
        ...
        templateParams: Object.keys(templateParams).length
          ? templateParams
          : undefined,
      });
      ...
      notifications.show({
        ...
        message:
          targetMode === "group"
            ? "Alerts applied to target group"
            : "Alerts applied to target key",
        color: "green",
  ...
  const categories = Array.from(
    new Set(groups.map((g) => getGroupCategory(g))),
  );
            ...
            {subscribedGroups.length === 0 && (
              <Center h={200}>
                <Stack align="center" gap="md">
                  ...
                  <Button variant="light" onClick={() => setActiveTab("all")}>
                    Browse Groups
                  </Button>
                </Stack>
              </Center>
      ...
      >
        <Stack gap="md">
          ...
          {targetMode === "group" ? (
            targetGroups.length === 0 ? (
              <Alert icon={<IconAlertCircle size={16} />} color="yellow">
                No matching target groups available. Create a group first, or
                target a single key.
              </Alert>
            ) : (
              <Select
            ...
            <Autocomplete
              ...
              placeholder={
                templatesResponse
                  ? getTargetKeyPlaceholder(templatesResponse.alert_type)
                  : "Type a target key…"
              }
          ...
          {isLoadingTemplates ? (
            ...
            <Text size="sm" c="dimmed">
              No required settings for this group.
            </Text>
          ) : (
            <Stack gap="sm">
              {requiredVariables.map((variable) => {
                  ...
                  return (
                    <Card key={variableId} withBorder padding="sm" radius="md">
                      <Group justify="space-between" align="center">
                        ...
                        <SegmentedControl
                          value={
                            (templateParams[variableId]
                              ? "true"
                              : "false") as string
                          }
                          onChange={(value) =>
                            setTemplateParam(variableId, value === "true")
                          }
                          data={[
                            { value: "true", label: "On" },
                ...
                if (variableType === "enum") {
                  return (
                    <Select
                      ...
                      value={
                        (templateParams[variableId] as string | undefined) ||
                        null
                      }
                      onChange={(value) => setTemplateParam(variableId, value)}
                      error={isMissing ? "Required" : undefined}
                ...
                return (
                  <TextInput
                    ...
                    onChange={(e) =>
                      setTemplateParam(variableId, e.currentTarget.value)
                    }
                    error={isMissing ? "Required" : undefined}
                  />
                );
