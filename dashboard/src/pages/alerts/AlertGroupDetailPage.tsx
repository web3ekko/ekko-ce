...
function getGroupCategory(group: GenericGroup | null): string {
  ...
  return typeof category === "string" && category.trim().length
    ? category
    : "Other";
}
...
export function AlertGroupDetailPage() {
  ...
  const [isLoadingAvailableTemplates, setIsLoadingAvailableTemplates] =
    useState(false);
  ...
  const compatibleAvailableTemplates = useMemo(() => {
    const eligible = availableTemplates.filter(
      (t) => getTemplateTargetAlertType(t) === groupAlertType,
    );
    ...
    return searched.map((t) => {
      ...
      if (existingTemplateIds.has(t.id)) {
        ...
        t.template_type.toLowerCase() !== baselineTemplateType
      ) {
        disabledReason = `Template type mismatch (${t.template_type} ≠ ${baselineTemplateType})`;
      } else if (
        baselineRequiredIds &&
        !setsEqual(getTemplateRequiredParamIds(t), baselineRequiredIds)
      ) {
  ...
  const loadAvailableTemplates = async () => {
    ...
    try {
      ...
      const results: ApiAlertTemplate[] = Array.isArray(payload)
        ? payload
        : payload?.results || [];
  ...
  const handleAddTemplate = async (templateId: string) => {
    ...
    try {
      ...
      const backendMessage =
        (err?.response?.data?.members &&
          Array.isArray(err.response.data.members) &&
          err.response.data.members.join("\n")) ||
        err?.message ||
  ...
  return (
    <Container size="xl" py="xl">
      ...
      <Modal
        ...
        onClose={addTemplateHandlers.close}
        title="Add Templates"
        size="xl"
      >
        <Stack gap="md">
          ...
          {isLoadingAvailableTemplates ? (
            ...
            <SimpleGrid cols={{ base: 1, md: 2 }} spacing="md">
              {compatibleAvailableTemplates.map(
