...
export function MarketplacePage() {
  ...
  useEffect(() => {
    ...
    const loadMarketplace = async () => {
      ...
      try {
        ...
        if (isActive) {
          setError(
            loadError?.message || "Unable to load marketplace templates",
          );
        }
      } finally {
        if (isActive) {
  ...
  const handleUseTemplate = (template: AlertTemplateSummary) => {
    ...
    navigate(
      `/dashboard/alerts?create=true&template_id=${template.id}&template_version=${template.latest_template_version}`,
    );
  };
      ...
      <Stack gap="lg">
        <Group justify="space-between" align="flex-start" wrap="wrap">
          <div>
            ...
            <Text c="dimmed" mt={4}>
              Explore alert plans curated for monitoring wallets, protocols, and
              networks.
            </Text>
          </div>
