...
function stageToMessage(stage: unknown): string | null {
  ...
  if (
    ...
    return "Checking chain support...";
  if (s === "draft_plan" || s === "draft_template")
    return "Drafting alert logic...";
  ...
  const submitJob = useCallback(
    async (
      ...
      const response: ParseJobResponse =
        await alertsApiService.parseNaturalLanguageJob(
          description,
          clientRequestId,
          pipelineId,
          context,
      ...
      timeoutRef.current = setTimeout(async () => {
        ...
        const fallback: ParseResultResponse =
          await alertsApiService.getParseResult(activeJobRef.current);
        ...
        const message =
          fallback.error || "NLP processing timed out. Please retry.";
        setStatus("error");
        setError(message);
        setProgress(null);
        options.onError?.(message);
