...
export function useAuthApi() {
  ...
  const signIn = useCallback(
    async (values: SignInCredential): Promise<{ status: Status; message: string } | undefined> => {
      setLoading(true);
      setError(null);
  ...
  const signUp = useCallback(
    async (values: SignUpCredential): Promise<{ status: Status; message: string } | undefined> => {
      setLoading(true);
      setError(null);
