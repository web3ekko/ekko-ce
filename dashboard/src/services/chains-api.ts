...
class ChainsApiService {
  async list(): Promise<PaginatedResponse<ChainInfo>> {
    try {
      const response = await httpClient.get<PaginatedResponse<ChainInfo>>(
        API_ENDPOINTS.CHAINS.LIST,
      );
  ...
  async listSubChains(chainId: string): Promise<SubChainInfo[]> {
    try {
      const response = await httpClient.get<SubChainInfo[]>(
        API_ENDPOINTS.CHAINS.SUB_CHAINS(chainId),
      );
      return response.data;
    } catch (error: unknown) {
