...
class NotificationsApiService {
  ...
  ): Promise<PaginatedResponse<NotificationChannelEndpoint>> {
    try {
      const response = await httpClient.get<
        PaginatedResponse<NotificationChannelEndpoint>
      >(API_ENDPOINTS.NOTIFICATIONS.CHANNELS, params);
  ...
  async createSlackChannel(
    data: CreateSlackChannelRequest,
  ): Promise<NotificationChannelEndpoint> {
    try {
      const payload = {
        channel_type: "slack",
        label: data.label,
  ...
  async createWebhookChannel(
    data: CreateWebhookChannelRequest,
  ): Promise<NotificationChannelEndpoint> {
    try {
      const payload = {
  ...
  // Create SMS channel
  async createSMSChannel(
    data: CreateSMSChannelRequest,
  ): Promise<NotificationChannelEndpoint> {
    try {
      const payload = {
        channel_type: "sms",
  ...
  async testChannel(
    ...
    try {
      ...
      const response = await httpClient.post<{
        success: boolean;
        message: string;
      }>(`${API_ENDPOINTS.NOTIFICATIONS.CHANNELS}${channelId}/test/`, payload);
      return response.data;
    } catch (error: any) {
