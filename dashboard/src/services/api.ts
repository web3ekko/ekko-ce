...
class ApiClient {
  ...
  async signupVerifyCode(data: {
    email: string;
    code: string;
  }): Promise<SignupCompleteResponse> {
    return this.request<SignupCompleteResponse>(
  ...
  async signinPasskeyBegin(data: {
    email: string;
  }): Promise<{ challenge: string; allowCredentials: any[]; rpId: string }> {
  ...
  async signinPasskeyComplete(
    data: PasskeyAuthRequest,
  ): Promise<AuthSuccessResponse> {
    return this.request<AuthSuccessResponse>(
      "/api/auth/signin/passkey/complete/",
      {
        method: "POST",
        body: JSON.stringify(data),
      },
  ...
  async signinEmailSendCode(data: {
    email: string;
  ...
  async signinEmailVerifyCode(data: {
    email: string;
    code: string;
  }): Promise<AuthSuccessResponse> {
    return this.request<AuthSuccessResponse>(
  ...
  async verifyMagicLink(
    ...
    return this.signinEmailVerifyCode({
      email: data.email,
      code: data.token || data.code || "",
    });
  }
  ...
  async login(data: LoginRequest): Promise<LoginResponse> {
    ...
    if (options.has_passkey) {
      // Start passkey flow
      return this.signinPasskeyBegin({
        email: data.email,
      }) as unknown as LoginResponse;
    } else {
      // Start email code flow
      return this.signinEmailSendCode({
        email: data.email,
      }) as unknown as LoginResponse;
    }
  }
  ...
  async registerPasskey(data: {
    credential_data: any;
    device_info?: any;
  }): Promise<{ success: boolean; passkey_id: string }> {
  ...
  async validateToken(): Promise<{ valid: boolean; user: User }> {
    return this.request<{ valid: boolean; user: User }>(
  ...
  async getFirebaseToken(): Promise<{
    firebase_token: string;
    expires_in: number;
  }> {
    return this.request<{ firebase_token: string; expires_in: number }>(
