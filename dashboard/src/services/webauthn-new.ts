...
class WebAuthnNewService {
  ...
  async createPasskey(authToken: string, options: any): Promise<PasskeyDevice> {
    ...
    try {
      ...
      const completeResponse = await fetch(
        `${this.apiUrl}/api/passkeys/register/complete/`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  async authenticatePasskey(options: any): Promise<any> {
    ...
    try {
      ...
      const timeoutPromise = new Promise<never>((_, reject) => {
        setTimeout(() => {
          reject(
            new Error("Authentication timed out. No passkeys were selected."),
          );
        }, timeoutMs);
      });
      ...
      const completeResponse = await fetch(
        `${this.apiUrl}/api/passkeys/authenticate/complete/`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
  ...
  async deleteDevice(authToken: string, deviceId: string): Promise<void> {
    const response = await fetch(
      `${this.apiUrl}/api/passkeys/devices/${deviceId}/delete/`,
      {
        method: "DELETE",
        headers: {
          Authorization: `Token ${authToken}`,
  ...
  async updateDevice(
    ...
    const response = await fetch(
      `${this.apiUrl}/api/passkeys/devices/${deviceId}/`,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
  ...
  private getDeviceName(): string {
    ...
    if (/Chrome/.test(ua) && !/Edg/.test(ua))
      return "Chrome on " + this.getOS();
    if (/Safari/.test(ua) && !/Chrome/.test(ua))
      return "Safari on " + this.getOS();
