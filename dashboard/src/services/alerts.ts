...
class AlertApiService {
  ...
  async getTemplates(
    ...
    return apiClient.request<AlertTemplate[]>(
      `/api/alert-templates/?${params}`,
    );
  }
  ...
  async getAlertMetrics(alertId: string, period = "7d"): Promise<AlertMetrics> {
    return apiClient.request<AlertMetrics>(
      `/api/alerts/${alertId}/metrics/?period=${period}`,
    );
  }
  ...
  async getNotificationChannels(
    ...
    return apiClient.request<NotificationChannel[]>(
      `/api/notification-channels/${params}`,
    );
  }
  ...
  async testNotificationChannel(
    ...
    return apiClient.request<{
      ...
      method: "POST",
      body: JSON.stringify({
        message: message || "Test notification from Ekko Dashboard",
      }),
    });
  }
  ...
  async exportAlerts(alertIds?: string[]): Promise<Blob> {
    ...
    const response = await fetch(
      `${apiClient.getBaseURL()}/api/alerts/export/${params}`,
      {
        headers: {
  ...
  async importAlerts(file: File): Promise<{
    ...
    const response = await fetch(
      `${apiClient.getBaseURL()}/api/alerts/import/`,
      {
        method: "POST",
        headers: {
          Authorization: `Bearer ${apiClient.getAccessToken()}`,
