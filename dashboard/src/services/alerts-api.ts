...
export interface AlertTemplateLatestResponse {
  success: boolean;
  template?:
    (AlertTemplateSummary & { latest_version_bundle?: unknown }) | null;
  bundle?: {
    template_version: number;
    spec_hash: string;
...
class AlertsApiService {
  ...
  ): Promise<PaginatedResponse<SimpleAlert>> {
    try {
  ...
  async parseNaturalLanguage(description: string): Promise<NLPParseResult> {
    try {
      ...
      return {
        success: false,
        error:
          error.response?.data?.error || error.message || "NLP parsing failed",
      };
    }
  }
  ...
  async saveTemplateFromJob(
    request: AlertTemplateSaveRequest,
  ): Promise<AlertTemplateSaveResponse> {
    try {
  ...
  ): Promise<PaginatedResponse<AlertTemplateSummary>> {
    try {
      const response = await httpClient.get<
        PaginatedResponse<AlertTemplateSummary>
      >(API_ENDPOINTS.ALERTS.TEMPLATES, params);
      return response.data;
    } catch (error: any) {
  ...
  async getTemplateLatest(
    templateId: string,
  ): Promise<AlertTemplateLatestResponse> {
    try {
