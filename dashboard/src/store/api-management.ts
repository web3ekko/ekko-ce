...
interface ApiManagementState {
  ...
  createWebhook: (
    data: Omit<
      WebhookConfig,
      "id" | "created_at" | "delivery_count" | "last_delivery" | "status"
    >,
  ) => Promise<void>;
  ...
  updateTestRequest: (
    updates: Partial<ApiManagementState["testRequest"]>,
  ) => void;
...
const mapUsage = (record: ApiUsageRecord): ApiUsage => {
  const successRate =
    record.requests > 0
      ? (record.requests - record.errors) / record.requests
      : 0;
  return {
    date: record.date,
...
const mapWebhook = (
  endpoint: NotificationChannelEndpoint,
...
export const useApiManagementStore = create<ApiManagementState>((set, get) => ({
    ...
    try {
      ...
      const stats = await Promise.all(
        endpoints.map(async (endpoint) => {
          try {
            const result = await notificationsApiService.getChannelStats(
              endpoint.id,
            );
      ...
      const mapped = endpoints.map((endpoint, index) =>
        mapWebhook(endpoint, stats[index] || 0),
      );
  ...
  revokeApiKey: async (id) => {
    ...
    set((state) => ({
      apiKeys: state.apiKeys.map((key) =>
        key.id === id ? mapApiKey(updated) : key,
      ),
    }));
  },
  ...
  updateWebhook: async (id, updates) => {
    ...
    const stats = await notificationsApiService
      .getChannelStats(id)
      .catch(() => ({ success_count: 0, failure_count: 0 }));
    const deliveryCount = stats.success_count + stats.failure_count;
    set((state) => ({
      webhooks: state.webhooks.map((webhook) =>
        webhook.id === id ? mapWebhook(response, deliveryCount) : webhook,
      ),
    }));
  },
  ...
  setSelectedEndpoint: (endpoint) => {
    const state = get();
    const endpointData = state.endpoints.find(
      (e) => `${e.method} ${e.path}` === endpoint,
    );
    ...
    if (endpointData) {
      set({
        ...
        testRequest: {
          ...
          body: endpointData.example_request
            ? JSON.stringify(endpointData.example_request, null, 2)
            : "",
        },
      });
    } else {
