...
export interface WalletStoreState {
  ...
  updateAccountWallet: (
    walletKey: string,
    updates: { label?: string; owner_verified?: boolean },
  ) => Promise<void>;
  createWalletGroup: (data: {
    name: string;
    description?: string;
...
function parseWalletKey(walletKey: string): {
  network: string;
  subnet: string;
  address: string;
} {
...
export const useWalletStore = create<WalletStoreState>((set, get) => ({
  ...
  removeAccountWallet: async (walletKey) => {
    const accountsGroup =
      get().accountsGroup || (await groupsApiService.getAccountsGroup());
    if (!accountsGroup) {
  ...
  updateAccountWallet: async (walletKey, updates) => {
    ...
    set({ isLoading: true, error: null });
    try {
      await groupsApiService.updateMembers(accountsGroup.id, {
  ...
  createWalletGroup: async ({ name, description, settings }) => {
    ...
    try {
      ...
      const refreshed = await groupsApiService.getGroupsByType(
        GroupType.WALLET,
      );
