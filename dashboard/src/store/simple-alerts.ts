...
function mapApiAlertToSimpleAlert(alert: any): SimpleAlert {
  ...
  return {
    ...
    target_keys: Array.isArray(alert.target_keys)
      ? alert.target_keys
      : undefined,
    priority: alert.priority,
    source_type: alert.template
      ? "template"
      : alert.nl_description
        ? "natural_language"
        : "manual",
...
function applyNotificationHistory(
  alerts: SimpleAlert[],
  ...
  historyItems.forEach((item) => {
    ...
    const current = stats.get(item.alert_id) || {
      last_triggered: item.created_at,
      count: 0,
    };
    current.count += 1;
    if (
      new Date(item.created_at).getTime() >
      new Date(current.last_triggered).getTime()
    ) {
  ...
  return alerts.map((alert) => {
    ...
    const lastTriggered =
      existingLast &&
      new Date(existingLast).getTime() > new Date(stat.last_triggered).getTime()
        ? existingLast
        : stat.last_triggered;
...
interface SimpleAlertState {
  ...
  runTemplatePreviewFromJob: (
    request: AlertTemplatePreviewRequest,
  ) => Promise<void>;
  clearPreview: () => void;
}
...
export const useSimpleAlerts = create<SimpleAlertState>((set, get) => ({
  ...
  loadAlerts: async () => {
    ...
    try {
      ...
      let mappedAlerts: SimpleAlert[] = (response.results || []).map(
        mapApiAlertToSimpleAlert,
      );

      try {
        const history = await notificationsApiService.getHistory({
          limit: 200,
        });
        mappedAlerts = applyNotificationHistory(
          mappedAlerts,
          history.results || [],
        );
      } catch (historyError) {
        console.warn(
          "Failed to load notification history for trigger stats:",
          historyError,
        );
      }
  ...
  updateAlert: async (id, updates) => {
    try {
      ...
      if (updates.description !== undefined)
        apiUpdates.nl_description = updates.description;
      if (updates.enabled !== undefined) apiUpdates.enabled = updates.enabled;

      const updatedAlert = await alertsApiService.updateAlert(
        id,
        apiUpdates as any,
      );
  ...
  deleteAlert: async (id) => {
    try {
      ...
      set((state) => ({
        ...
        selectedAlerts: state.selectedAlerts.filter(
    ...
    try {
      ...
      set((state) => ({
        alerts: state.alerts.map((a) =>
          a.id === id ? mapApiAlertToSimpleAlert(updatedAlert) : a,
        ),
      }));
    } catch (error) {
    ...
    try {
      ...
      set((state) => ({
        alerts: state.alerts.filter(
          (alert) => !state.selectedAlerts.includes(alert.id),
        ),
        selectedAlerts: [],
      }));
    } catch (error) {
  ...
  runInlinePreview: async (spec, config = {}, alertType = "wallet") => {
    ...
    try {
      const result = await alertsApiService.previewInline(
        spec,
        config,
        alertType,
      );
