...
import walletNicknamesApiService, {
  type WalletNickname,
} from "../services/wallet-nicknames-api";
  ...
  (set, get) => ({
    ...
    loadWalletNicknames: async () => {
      ...
      try {
        ...
        set({
          error: "Failed to load wallet nicknames",
          isLoadingNicknames: false,
        });
      }
    },
    ...
    getWalletNickname: (address, chainId) => {
      ...
      if (typeof chainId === "number") {
        const record =
          get().walletNicknameByKey[buildAddressChainKey(addressKey, chainId)];
