...
def generate_tokens_for_user(user):
    ...
    try:
        ...
        cache_service.cache_token(
            token=token, user_id=str(user.id), expiry=instance.expiry
        )
...
def generate_firebase_custom_token(user):
    ...
    try:
        ...
        logger.warning(
            f"Failed to generate Firebase custom token for {user.email}: {e}"
        )
    return None
...
def check_account_status(request):
    ...
    try:
        ...
        if user.is_active:
            ...
            return Response(
                {"status": "inactive_account", "message": "Inactive account found"}
            )
    except User.DoesNotExist:
        return Response({"status": "no_account", "message": "No account found"})
...
def signup_begin(request):
    ...
    """
    logger.info(
        f"🔥 SIGNUP BEGIN - Request received from {request.META.get('REMOTE_ADDR')}"
    )
    ...
    if firebase_auth_manager.is_available():
        try:
            ...
            firebase_uid = (
                f"fallback_{secrets.token_hex(8)}_{int(timezone.now().timestamp())}"
            )
    else:
    ...
    if user:
        ...
        user.save()
        logger.info(
            f"🔥 Updated existing inactive user with Firebase UID: {firebase_uid}"
        )
    else:
        ...
        logger.info(
            f"🔥 Django user created (ID: {user.id}) with Firebase UID: {firebase_uid}"
        )
    ...
    try:
        ...
        if django_settings.DEBUG:
            ...
            logger.info(
                f"🔑 DEV MODE - Signup verification code for {email}: {verification_code.code}"
            )
...
def signup_verify_code(request):
    ...
    if not verification_code:
        ...
        recent_code = (
            EmailVerificationCode.objects.filter(email=email, purpose="signup")
            .order_by("-created_at")
            .first()
        )
        ...
        return Response(
            {"error": "Invalid code", "error_type": "invalid"},
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    logger.info(
        f"🔑 Created and cached auth token for {user.email} after email verification"
    )
...
def signup_complete(request):
    ...
    try:
        ...
        if user.is_active:
            return Response(
                {"error": "Account already active"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        if ALLAUTH_WEBAUTHN_AVAILABLE:
            try:
                ...
                logger.info(
                    f"🔥 Passkey created and account activated for: {user.email}"
                )
            except Exception as e:
                logger.error(
                    f"Failed to complete passkey registration for {user.email}: {e}"
                )
                return Response(
...
def resend_code(request):
    ...
    if purpose not in ["signup", "signin", "recovery"]:
        return Response(
            {"error": "Invalid purpose"}, status=status.HTTP_400_BAD_REQUEST
        )
...
def signin_passkey_complete(request):
    ...
    if not credential_data:
        return Response(
            {"error": "Credential data is required"}, status=status.HTTP_400_BAD_REQUEST
        )
...
def signin_email_verify_code(request):
    ...
    if not verification_code:
        ...
        recent_code = (
            EmailVerificationCode.objects.filter(email=email, purpose="signin")
            .order_by("-created_at")
            .first()
        )
...
def account_recovery_verify(request):
    ...
    if not verification_code:
        return Response(
            {"error": "Invalid or expired code", "error_type": "invalid"},
            status=status.HTTP_400_BAD_REQUEST,
        )
    ...
    try:
        ...
        if ALLAUTH_WEBAUTHN_AVAILABLE:
            try:
                ...
                logger.error(
                    f"Failed to complete passkey registration in recovery: {e}"
                )
                return Response(
...
def logout_user(request):
    ...
    try:
        ...
        logger.info(
            f"Invalidated {invalidated} Knox tokens from cache for user {user.id}"
        )
    except Exception as e:
    ...
    try:
        ...
        if "credential_data" not in request.data:
            ...
            try:
                ...
                if not request.session.session_key:
                    ...
                    logger.info(
                        f"[SIGNUP PASSKEY BEGIN] Created new session: {request.session.session_key}"
                    )
                ...
                if session_state:
                    ...
                    logger.info(
                        f"[SIGNUP PASSKEY BEGIN] Stored WebAuthn state in cache for user {user.id}"
                    )
                    logger.info(
                        f"[SIGNUP PASSKEY BEGIN] State keys: {list(session_state.keys()) if isinstance(session_state, dict) else 'Not a dict'}"
                    )
                else:
                    logger.warning(
                        f"[SIGNUP PASSKEY BEGIN] No WebAuthn state found in session"
                    )
                ...
                logger.error(
                    f"Failed to begin signup passkey registration for {user.email}: {e}"
                )
                return Response(
        ...
        # Complete registration - process WebAuthn response
            ...
            try:
                ...
                # Debug logging
                logger.info(
                    f"[SIGNUP PASSKEY COMPLETE] User: {user.email}, ID: {user.id}"
                )
                logger.info(
                    f"[SIGNUP PASSKEY COMPLETE] credential_data type: {type(credential_data)}"
                )
                logger.info(
                    f"[SIGNUP PASSKEY COMPLETE] credential_data keys: {list(credential_data.keys()) if isinstance(credential_data, dict) else 'Not a dict'}"
                )
                ...
                if cached_state:
                    logger.info(
                        f"[SIGNUP PASSKEY COMPLETE] Retrieved WebAuthn state from cache"
                    )
                    ...
                    request.session.save()
                    logger.info(
                        f"[SIGNUP PASSKEY COMPLETE] Restored WebAuthn state to session"
                    )
                else:
                    logger.warning(
                        f"[SIGNUP PASSKEY COMPLETE] No cached WebAuthn state found"
                    )
                ...
                try:
                    with request_context(request):
                        ...
                        logger.info(
                            f"[SIGNUP PASSKEY COMPLETE] Registration result type: {type(result)}"
                        )
                        logger.info(
                            f"[SIGNUP PASSKEY COMPLETE] Registration result: {result}"
                        )
                        ...
                        if hasattr(result, "id") and hasattr(result, "user"):
                            ...
                            logger.info(
                                f"[SIGNUP PASSKEY COMPLETE] Found authenticator after registration: {authenticator}"
                            )
                            ...
                            if not authenticator and hasattr(result, "counter"):
                                ...
                                logger.info(
                                    f"[SIGNUP PASSKEY COMPLETE] Attempting to create authenticator from AuthenticatorData"
                                )

                                # result is the AuthenticatorData object itself
                                # Check if it has credential_data attribute (for attested credential data)
                                if (
                                    hasattr(result, "credential_data")
                                    and result.credential_data
                                ):
                                    ...
                                    logger.info(
                                        f"[SIGNUP PASSKEY COMPLETE] Created authenticator manually from AuthenticatorData: {authenticator.id}"
                                    )
                                else:
                                    logger.error(
                                        f"[SIGNUP PASSKEY COMPLETE] AuthenticatorData has no credential_data attribute"
                                    )
                    ...
                    logger.error(
                        f"[SIGNUP PASSKEY COMPLETE] Error during registration: {e}"
                    )
                    logger.error(
                        f"[SIGNUP PASSKEY COMPLETE] Error type: {type(e).__name__}"
                    )
                    logger.error(
                        f"[SIGNUP PASSKEY COMPLETE] Credential data structure: {credential_data}"
                    )
                    ...
                    try:
                        ...
                        if authenticator:
                            logger.info(
                                f"[SIGNUP PASSKEY COMPLETE] Found existing authenticator: {authenticator.id}"
                            )
                            # Clean up the cached state
                            cache.delete(cache_key)
                        else:
                            ...
                            logger.info(
                                f"[SIGNUP PASSKEY COMPLETE] Created authenticator manually: {authenticator.id}"
                            )
                        ...
                        logger.error(
                            f"[SIGNUP PASSKEY COMPLETE] Manual registration also failed: {manual_error}"
                        )
                        # Log the full traceback for debugging
                        import traceback

                        logger.error(
                            f"[SIGNUP PASSKEY COMPLETE] Traceback: {traceback.format_exc()}"
                        )
                ...
                if not authenticator:
                    logger.error(
                        f"[SIGNUP PASSKEY COMPLETE] No authenticator was created"
                    )
                    return Response(
                ...
                logger.error(
                    f"Failed to complete signup passkey registration for {user.email}: {e}"
                )
                return Response(
    ...
    try:
        ...
        if "credential_data" not in request.data:
            ...
            try:
                ...
                logger.error(
                    f"Failed to begin passkey registration for {user.email}: {e}"
                )
                return Response(
                    {"error": "Failed to generate registration options"},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        ...
        if "credential_data" not in request.data:
            ...
            if not email:
                try:
                    ...
                    rp_id = getattr(
                        settings, "WEBAUTHN_RP_ID", request.get_host().split(":")[0]
                    )
                ...
                if stored_challenge:
                    ...
                    if user_handle:
                        ...
                        try:
                            ...
                            user = User.objects.get(id=user_id)
                            logger.info(
                                f"Identified user from userHandle: {user.email}"
                            )
                        except Exception as e:
                            logger.error(
                                f"Failed to identify user from userHandle: {e}"
                            )
                            user = None
                    else:
                        # No userHandle provided - cannot identify user
                        logger.error(
                            "No userHandle in passwordless authentication response"
                        )
                        user = None
                    ...
                    user = webauthn_auth.complete_authentication(
                        request, credential_data
                    )
...
def list_passkeys(request):
    ...
    return Response({"passkeys": passkey_data, "count": len(passkey_data)})
...
def delete_passkey(request, passkey_id):
    ...
    try:
        authenticator = Authenticator.objects.get(
            id=passkey_id, user=request.user, type=Authenticator.Type.WEBAUTHN
        )

        device_name = (
            authenticator.data.get("device_name", "Unknown Device")
...
def knox_token_info(request):
    ...
    if not auth_header.startswith("Token "):
        return Response(
            {"error": "No Knox token provided"}, status=status.HTTP_400_BAD_REQUEST
        )
...
def knox_list_tokens(request):
    ...
    return Response({"tokens": token_data, "count": len(token_data)})
...
def firebase_status(request):
    ...
    try:
        ...
        return Response(
            {
                ...
                "project_id": getattr(settings, "FIREBASE_ADMIN_CONFIG", {}).get(
                    "project_id", ""
                ),
                "features_enabled": {
        ...
        return Response(
            {"firebase_configured": False, "firebase_available": False, "error": str(e)}
        )
...
def webauthn_status(request):
    ...
    try:
        ...
        return Response(
            {
                ...
                "passkey_signup_enabled": getattr(
                    settings, "MFA_PASSKEY_SIGNUP_ENABLED", False
                ),
                "passkey_login_enabled": getattr(
        ...
        return Response({"webauthn_configured": False, "error": str(e)})
...
def register_push_token(request):
    ...
    device_type = serializer.validated_data.get(
        "device_type", "ios" if token_type == "apns" else "android"
    )
        ...
        return Response(
            {
                ...
                "device": {
                    ...
                    "token_updated_at": device.token_updated_at.isoformat()
...
def toggle_device_push(request, device_id):
    ...
    try:
        ...
        logger.info(
            f"Push notifications {'enabled' if enabled else 'disabled'} for device {device_id}"
        )
