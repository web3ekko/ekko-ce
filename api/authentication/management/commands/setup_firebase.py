    ...
    def check_firebase_config(self):
        ...
        try:
            ...
            if is_available:
                ...
                self.stdout.write(
                    f'   Project ID: {firebase_config.get("project_id", "Not set")}'
                )
                self.stdout.write(
                    f'   Credentials Path: {firebase_config.get("credentials_path", "Not set")}'
                )
                self.stdout.write(
                    f'   Service Account Key: {"Set" if firebase_config.get("service_account_key") else "Not set"}'
                )
                self.stdout.write(
                    f'   Web API Key: {"Set" if web_config.get("apiKey") else "Not set"}'
                )
                self.stdout.write(
                    f'   Auth Domain: {web_config.get("authDomain", "Not set")}'
                )
                ...
                self.stdout.write(
                    "   Authentication will work with Django-only features"
                )
                ...
                self.stdout.write(
                    "   2. Enable Authentication with Email/Password and Email Link"
                )
            ...
            self.stdout.write(
                self.style.ERROR(f"❌ Error checking Firebase configuration: {e}")
            )

    def setup_firebase_config(self, options):
        """Set up Firebase configuration"""
        self.stdout.write(
            self.style.HTTP_INFO("🔧 Setting up Firebase Configuration...")
        )
        ...
        if options["service_account"]:
            ...
            if os.path.exists(service_account_path):
                ...
                try:
                    ...
                    self.stdout.write(
                        self.style.WARNING(
                            f"⚠️  Could not read service account file: {e}"
                        )
                    )
            else:
                self.stdout.write(
        ...
        if options["auth_domain"]:
            ...
            ] = f"{options['project_id']}.firebaseapp.com"

        if not env_updates:
            self.stdout.write(
                self.style.WARNING("⚠️  No configuration options provided")
            )
        ...
        self.stdout.write(
            "   3. Run: python manage.py test apps.api.tests.test_authentication.AuthenticationSystemArchitectureTest.test_firebase_integration_configuration"
        )
