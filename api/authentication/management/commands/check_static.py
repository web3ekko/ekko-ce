...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if "whitenoise.middleware.WhiteNoiseMiddleware" in settings.MIDDLEWARE:
            self.stdout.write(
                self.style.SUCCESS("✓ WhiteNoise middleware is installed")
            )
        else:
            self.stdout.write(
                self.style.ERROR("✗ WhiteNoise middleware is NOT installed")
            )
        ...
        if os.path.exists(settings.STATIC_ROOT):
            self.stdout.write(
                self.style.SUCCESS(f"✓ STATIC_ROOT exists: {settings.STATIC_ROOT}")
            )
            ...
            if os.path.exists(admin_path):
                self.stdout.write(
                    self.style.SUCCESS(f"✓ Admin static files exist in STATIC_ROOT")
                )
            else:
                self.stdout.write(
                    self.style.WARNING(f"⚠ Admin static files not found in STATIC_ROOT")
                )
        else:
            self.stdout.write(
                self.style.WARNING(
                    f"⚠ STATIC_ROOT does not exist: {settings.STATIC_ROOT}"
                )
            )
