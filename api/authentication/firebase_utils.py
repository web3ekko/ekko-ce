...
class FirebaseAuthManager:
    ...
    def generate_password_reset_link(
        self, email: str, action_code_settings: Optional[Dict[str, Any]] = None
    ) -> str:
        """
    ...
    def generate_email_verification_link(
        ...
        if not self.is_available():
            logger.error(
                f"🔥 Firebase not available for email verification link: {email}"
            )
            raise ImproperlyConfigured("Firebase Admin SDK is not properly configured")
        ...
        try:
            ...
            logger.info(
                f"🔥 Action code settings provided: {action_code_settings is not None}"
            )
            ...
            logger.info(
                f"🔥 Successfully generated email verification link for: {email}"
            )
            logger.info(
                f"🔥 Generated link: {link[:50]}..."
    ...
    def create_custom_token(
        self, uid: str, additional_claims: Dict[str, Any] = None
    ) -> bytes:
        """
...
def create_action_code_settings(
    ...
    action_code_settings = ActionCodeSettings(
        url=continue_url, handle_code_in_app=handle_code_in_app
    )
