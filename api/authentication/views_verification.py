...
def signup_verify_code(request):
    ...
    try:
        ...
        logger.info(
            f"User get_or_create result - email: {email}, created: {created}, is_active: {user.is_active}"
        )

        if not created:
            # Existing user - mark email as verified and activate
            logger.info(
                f"Existing user found: {user.email}, is_active before: {user.is_active}"
            )
            ...
            logger.info(
                f"User after refresh: {user.email}, is_active: {user.is_active}"
            )

            # Double-check by querying directly
            db_user = User.objects.get(pk=user.pk)
            logger.info(
                f"Direct DB query - user: {db_user.email}, is_active: {db_user.is_active}"
            )
        else:
        ...
        logger.info(
            f"Final user state before response - email: {user.email}, is_active: {user.is_active}, is_email_verified: {user.is_email_verified}"
        )
...
def signin_email_send_code(request):
    ...
    return Response(
        {"success": True, "message": "Enter the code we sent to your email"}
    )
    ...
    if not verification_code:
        ...
        recent_code = (
            EmailVerificationCode.objects.filter(
                email=email, purpose="signin", used_at__isnull=True
            )
            .order_by("-created_at")
            .first()
        )
...
def resend_code(request):
    ...
    if is_rate_limited(
        rate_limit_key, max_attempts=max_attempts, window_minutes=window // 60
    ):
        return Response(
...
def recovery_request(request):
    ...
    return Response({"success": True, "message": response_message})
