...
class WebAuthnService:
    ...
    def __init__(self):
        ...
        self.allowed_origins = getattr(
            settings, "WEBAUTHN_ALLOWED_ORIGINS", [self.origin]
        )
        ...
        self.server = Fido2Server(
            self.rp, verify_origin=lambda origin: origin in self.allowed_origins
        )

        # Challenge settings
        self.challenge_timeout = getattr(
            settings, "WEBAUTHN_CHALLENGE_TIMEOUT", 300
        )  # 5 minutes
    ...
    def generate_registration_options(
        self, user: User, platform_only: bool = False
    ) -> Dict:
        """
        ...
        try:
            ...
            for device in user.passkey_devices.filter(is_active=True):
                existing_credentials.append(
                    PublicKeyCredentialDescriptor(
                        id=websafe_decode(device.credential_id), type="public-key"
                    )
                )
            ...
            logger.debug(
                f"User ID bytes: {user_id_bytes}, display name: {user_display_name}"
            )

            user_entity = PublicKeyCredentialUserEntity(
                id=user_id_bytes, name=user.email, display_name=user_display_name
            )
        except Exception as e:
        ...
        try:
            ...
            if hasattr(options, "public_key") and hasattr(
                ...
                if isinstance(challenge_str, str):
                    ...
                    challenge = options.public_key.challenge
                    logger.debug(
                        f"State has string challenge, using bytes from options instead"
                    )
                else:
                    challenge = challenge_str
            else:
                raise ValueError(
                    f"Could not find challenge in state or options. State: {state}, Options: {options}"
                )

            logger.debug(
                f"Challenge type: {type(challenge)}, length: {len(challenge) if hasattr(challenge, '__len__') else 'N/A'}"
            )
            ...
            if isinstance(state, dict):
                ...
                if "challenge" in state_to_store and isinstance(
                    state_to_store["challenge"], bytes
                ):
                    state_to_store["challenge"] = websafe_encode(
                        state_to_store["challenge"]
                    )
            else:
    ...
    def complete_registration(
        self, user: User, credential_data: Dict, device_name: Optional[str] = None
    ) -> PasskeyDevice:
        """
        ...
        try:
            ...
            logger.debug(
                f"clientDataJSON type: {type(client_data_json)}, value: {client_data_json}"
            )
            ...
            logger.debug(
                f"attestationObject type: {type(attestation_object)}, value: {attestation_object}"
            )
            ...
            try:
                ...
                logger.debug(
                    f"Decoded attestation object type: {type(decoded_att_obj)}, length: {len(decoded_att_obj)}"
                )
        ...
        try:
            ...
            logger.debug(
                f"Challenge user_id: {challenge_data['user_id']}, type: {type(challenge_data['user_id'])}"
            )
    ...
    def generate_authentication_options(
        self, user: Optional[User] = None, passwordless: bool = False
    ) -> Dict:
        """
    ...
    def complete_authentication(
        self, credential_data: Dict, user: Optional[User] = None
    ) -> Tuple[User, PasskeyDevice]:
        """
        ...
        client_data = CollectedClientData(
            websafe_decode(credential_data["response"]["clientDataJSON"])
        )
        auth_data = AuthenticatorData(
            websafe_decode(credential_data["response"]["authenticatorData"])
        )
        ...
        try:
            device = PasskeyDevice.objects.get(
                credential_id=websafe_encode(credential_id), is_active=True
            )
        ...
        logger.info(
            f"Successful authentication for user {auth_user.email} with device {device.id}"
        )
