...
def passkey_register_begin(request):
    ...
    try:
        ...
        options = service.generate_registration_options(
            user=request.user, platform_only=platform_only
        )
        ...
        logger.info(
            f"Authenticator selection: {options['publicKey'].get('authenticatorSelection', {})}"
        )

        return Response({"options": options, "success": True})
...
def passkey_register_complete(request):
    ...
    """
    logger.debug(
        f"Received registration completion request from user: {request.user.email}"
    )
    ...
    try:
        ...
        logger.info(
            f"Registered passkey device {device.id} for user {request.user.email}"
        )
    ...
    try:
        if email:
            ...
            try:
                ...
                return Response(
                    {"error": "No account found with this email", "success": False},
                    status=status.HTTP_404_NOT_FOUND,
                )

        service = WebAuthnService()
        options = service.generate_authentication_options(
            user=user, passwordless=passwordless
        )

        logger.info(f"Generated authentication options (passwordless={passwordless})")
        return Response(
            {"options": options, "success": True, "passwordless": passwordless}
        )
    except Exception as e:
...
def passkey_authenticate_complete(request):
    ...
    try:
        ...
        logger.info(
            f"Successful authentication for user {user.email} with device {device.id}"
        )
        ...
        return Response(
            {
                ...
                "token": token,
                "tokens": {"access": token, "refresh": token},  # Knox uses same token
                "device": PasskeyDeviceSerializer(device).data,
...
def list_passkey_devices(request):
    ...
    return Response({"success": True, "devices": serializer.data})
...
def delete_passkey_device(request, device_id):
    ...
    try:
        ...
        logger.info(
            f"Deactivated passkey device {device_id} for user {request.user.email}"
        )

        return Response({"success": True, "message": "Passkey removed successfully"})
...
def update_passkey_device(request, device_id):
    ...
    try:
        ...
        if name:
            ...
            logger.info(
                f"Updated passkey device {device_id} name for user {request.user.email}"
            )

            return Response(
                {"success": True, "device": PasskeyDeviceSerializer(device).data}
            )
        else:
            return Response(
