...
SECRET_KEY = env(
    "SECRET_KEY",
    default="django-insecure-z6ef2111lz@5qdua=2hmb5sakdv3d%#u0ng_wdbxke1l*v!18_",
)
...
if DEBUG:
    ...
    DATABASES = {
        "default": env.db_url(
            "DATABASE_URL", default="postgresql://ekko:ekko@localhost:5432/ekko_api"
        )
    }
...
ACCOUNT_SIGNUP_FIELDS = [
    ...
    "first_name*",
    "last_name*",
]  # No password fields for passwordless
...
MFA_WEBAUTHN_ALLOW_INSECURE_ORIGIN = env(
    "MFA_WEBAUTHN_ALLOW_INSECURE_ORIGIN", default=True
)  # For development
...
WEBAUTHN_CHALLENGE_TIMEOUT = env.int(
    "WEBAUTHN_CHALLENGE_TIMEOUT", default=300
)  # 5 minutes
...
NLP_SUPPORTED_NETWORKS = env.list(
    "NLP_SUPPORTED_NETWORKS", default=["ETH:mainnet", "AVAX:mainnet"]
)

# DuckLake Schema Registry Configuration (NATS-based)
DUCKLAKE_SCHEMA_NATS_SUBJECT = env(
    "DUCKLAKE_SCHEMA_NATS_SUBJECT", default="ducklake.schema.list"
)
DUCKLAKE_SCHEMA_NATS_TIMEOUT = env.int("DUCKLAKE_SCHEMA_NATS_TIMEOUT", default=5)
DUCKLAKE_SCHEMA_FALLBACK_ENABLED = env.bool(
    "DUCKLAKE_SCHEMA_FALLBACK_ENABLED", default=True
)
...
if env("FIREBASE_SERVICE_ACCOUNT_KEY"):
    ...
    if isinstance(service_account_key, dict):
        ...
        try:
            FIREBASE_ADMIN_CONFIG["service_account_key"] = json.loads(
                service_account_key
            )
    ...
    FIREBASE_WEB_CONFIG.update(
        {
            ...
            or f"{env('FIREBASE_PROJECT_ID')}.firebaseapp.com",
