...
class ChannelSettingsSerializer(serializers.Serializer):
    ...
    config = serializers.DictField(child=serializers.CharField(), allow_empty=True)
...
class QuietHoursSerializer(serializers.Serializer):
    ...
    end_time = serializers.RegexField(
        regex=r"^([01]?[0-9]|2[0-3]):[0-5][0-9]$", help_text="End time in HH:MM format"
    )
...
class UserNotificationSettingsSerializer(serializers.ModelSerializer):
    ...
    channels = serializers.DictField(
        child=ChannelSettingsSerializer(), allow_empty=True
    )
...
class NotificationTemplateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if channel in ["email", "slack", "websocket"] and not data.get(
            "subject_template"
        ):
            raise serializers.ValidationError(
...
class NotificationTestSerializer(serializers.Serializer):
    ...
    channel = serializers.ChoiceField(
        choices=["email", "slack", "sms", "telegram", "discord", "webhook", "websocket"]
    )
...
class NotificationChannelConfigSerializer(serializers.Serializer):
    ...
    def to_representation(self, instance):
        ...
        templates = {
            "email": {"enabled": True, "config": {"address": "user@example.com"}},
            "slack": {
                "enabled": True,
                "config": {
                    "channel": "#alerts",
            ...
            "sms": {"enabled": True, "config": {"phone": "+1234567890"}},
            "telegram": {"enabled": True, "config": {"chat_id": "123456789"}},
            "discord": {
                "enabled": True,
    ...
    def validate_config(self, value):
        ...
        if channel_type == "email":
            if "address" not in value:
                raise serializers.ValidationError(
                    "Email channel requires 'address' in config"
                )
            ...
            if "phone_number" not in value:
                raise serializers.ValidationError(
                    "SMS channel requires 'phone_number' in config"
                )

        elif channel_type == "telegram":
            if "chat_id" not in value:
                raise serializers.ValidationError(
                    "Telegram channel requires 'chat_id' in config"
                )
    ...
    def to_representation(self, instance):
        ...
        if request and hasattr(request, "user"):
            ...
            if team:
                ...
                is_admin = (
                    user.is_team_admin(team)
                    if hasattr(user, "is_team_admin")
                    else False
                ) or (user.is_org_admin() if hasattr(user, "is_org_admin") else False)

                # Mask config for non-admins
                if not is_admin:
                    data["config"] = self._mask_config(
    ...
    def _mask_config(self, channel_type, config):
        ...
        if channel_type == "email":
            ...
            if url:
                ...
                masked_config[
                    "url"
                ] = f"{parsed.scheme}://*****.{parsed.netloc.split('.')[-1]}/***"
    ...
    def validate_disabled_endpoints(self, value):
        ...
        if team_id:
            ...
            endpoint_count = NotificationChannelEndpoint.objects.filter(
                id__in=value, owner_type="team", owner_id=team_id
            ).count()
...
class NotificationChannelVerificationSerializer(serializers.ModelSerializer):
    ...
    endpoint_channel_type = serializers.CharField(
        source="endpoint.channel_type", read_only=True
    )
...
class VerificationCodeRequestSerializer(serializers.Serializer):
    ...
    def validate_endpoint_id(self, value):
        ...
        try:
            ...
            # Verify ownership
            if endpoint.owner_type == "user" and str(endpoint.owner_id) != str(
                request.user.id
            ):
                raise serializers.ValidationError(
                    "You don't have permission to verify this endpoint"
                )
                ...
                is_admin = (
                    request.user.is_team_admin(team)
                    if hasattr(request.user, "is_team_admin")
                    else False
                ) or (
                    request.user.is_org_admin()
                    if hasattr(request.user, "is_org_admin")
                    else False
                )

                if not is_admin:
                    raise serializers.ValidationError(
                        "Only team admins can verify team endpoints"
                    )
