...
class GenericGroupCreateSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if (
            ...
            and settings.get("system_key") == SYSTEM_GROUP_ACCOUNTS
        ):
            raise serializers.ValidationError(
                {
        ...
        if group_type == GroupType.ALERT and initial_members:
            ...
            temp_group = GenericGroup(group_type=group_type, settings=settings)
...
class GroupMemberSerializer(serializers.Serializer):
    ...
    label = serializers.CharField(
        required=False, allow_blank=True, help_text="Optional label for the member"
    )
    ...
    metadata = serializers.DictField(
        required=False, help_text="Optional metadata dictionary"
    )
...
class GroupSubscriptionSerializer(serializers.ModelSerializer):
    ...
    target_group_type = serializers.CharField(
        source="target_group.group_type", read_only=True
    )
    owner_email = serializers.EmailField(source="owner.email", read_only=True)
    ...
    def validate(self, data):
        ...
        effective_alert_type = (
            alert_group.get_alert_type() if alert_group else None
        ) or "wallet"
        templates = None
        ...
        if (
            target_group
            and target_group.owner_id != user.id
            and not target_group.is_public()
        ):
            raise serializers.ValidationError(
        ...
        if (
            alert_group
            and alert_group.owner_id != user.id
            and not alert_group.is_public()
        ):
            raise serializers.ValidationError(
                {
        ...
        if alert_group and alert_group.group_type == GroupType.ALERT:
            ...
            if template_ids:
                ...
                if (
                    alert_group.get_alert_type()
                    and derived_target_type != alert_group.get_alert_type()
                ):
                    raise serializers.ValidationError(
                        {
                            "alert_group": (
        ...
        if target_key:
            ...
            AlertValidationService.validate_targets(
                effective_alert_type, [normalized_target_key]
            )
        ...
        if alert_group:
            if target_group:
                existing = GroupSubscription.objects.filter(
                    alert_group=alert_group, target_group=target_group, owner=user
                )
            else:
                existing = GroupSubscription.objects.filter(
...
class UserWalletGroupSerializer(serializers.ModelSerializer):
    ...
    wallet_group_name = serializers.CharField(
        source="wallet_group.name", read_only=True
    )
...
class UserWalletGroupCreateSerializer(serializers.ModelSerializer):
    ...
    def validate_wallet_keys(self, value):
        ...
        for raw in value or []:
            if not isinstance(raw, str) or not raw.strip():
                raise serializers.ValidationError(
                    "wallet_keys must be non-empty strings"
                )
    ...
    def validate_callback(self, value):
        ...
        if value.owner_id != request_user.id:
            raise serializers.ValidationError(
                "callback must belong to the authenticated user"
            )
        return value
...
class UserWalletGroupImportSerializer(serializers.Serializer):
    ...
    merge_mode = serializers.ChoiceField(
        choices=[("append", "append"), ("replace", "replace")], default="append"
    )
    dedupe = serializers.BooleanField(default=True, required=False)
