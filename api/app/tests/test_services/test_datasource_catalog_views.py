    ...
    def test_address_transactions_count_24h_entry_is_allowlisted(self):
        ...
        entry = next(
            (
                e
                for e in runtime
                if e.get("catalog_id") == "ducklake.address_transactions_count_24h"
            ),
            None,
        )
        assert (
            entry is not None
        ), "expected ducklake.address_transactions_count_24h to be allowlisted"
        assert entry["routing"]["table"] == "address_transactions"
        assert entry["sql"]["dialect"] == "duckdb"
        assert entry["sql"]["param_order"] == [
            "target_keys",
            "chain_id",
            "subnet",
            "as_of",
