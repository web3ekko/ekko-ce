...
class TestWalletSearchAndStatsViews:
    def test_search_wallets_prefers_accounts_label_and_sets_url(self, api_client, user):
        ...
        wallet_in_accounts = Wallet.objects.create(
            blockchain=chain, subnet="mainnet", address="0xabc", name="Wallet A"
        )
        wallet_not_in_accounts = Wallet.objects.create(
            blockchain=chain, subnet="mainnet", address="0xdef", name="Wallet B"
        )
    ...
    def test_search_wallets_uses_wallet_nickname_when_no_accounts_label(
