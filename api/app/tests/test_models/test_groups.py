...
class TestGenericGroupSettingsValidation:
    ...
    def test_allows_subscriptions_default(self, user):
        ...
        group = GenericGroup.objects.create(
            group_type=GroupType.WALLET, name="Default Subscriptions", owner=user
        )
...
class TestGenericGroupHelperMethods:
    ...
    def test_member_count_sync(self, user):
        ...
        group = GenericGroup.objects.create(
            ...
            name="Count Test",
            owner=user,
            member_data={"members": {"key1": {}, "key2": {}, "key3": {}}},
        )
...
class TestGroupSubscriptionValidation:
    ...
    def test_subscription_same_group_not_allowed(self, user, alert_group):
        ...
        with pytest.raises(ValidationError) as exc_info:
            GroupSubscription.objects.create(
                alert_group=alert_group, target_group=alert_group, owner=user
            )

        assert "cannot be the same" in str(exc_info.value)

    def test_subscription_wallet_alert_to_wallet_group(
        self, user, alert_group, wallet_group
    ):
        """Wallet AlertGroup can subscribe to WalletGroup"""
        subscription = GroupSubscription.objects.create(
            alert_group=alert_group, target_group=wallet_group, owner=user
        )
    ...
    def test_subscription_nft_token_id_target_key_preserves_token_id(self, user):
        ...
        assert (
            subscription.target_key
            == "ETH:mainnet:0xabcdef000000000000000000000000000000000000:Token:ID-V1"
        )

    def test_subscription_requires_exactly_one_target(
        self, user, alert_group, wallet_group
    ):
    ...
    def test_subscription_blank_target_key_rejected(self, user, alert_group):
        ...
        assert "Provide exactly one of target_group or target_key" in str(
            exc_info.value
        )

    def test_subscription_wallet_alert_to_network_group_fails(
        self, user, alert_group, network_group
    ):
    ...
    def test_subscription_network_alert_to_network_group(self, user, network_group):
        ...
        subscription = GroupSubscription.objects.create(
            alert_group=network_alert_group, target_group=network_group, owner=user
        )
        assert subscription.id is not None
...
class TestUserWalletGroup:
    ...
    def test_create_user_wallet_group(self, provider_user, end_user):
        ...
        wallet_group = GenericGroup.objects.create(
            group_type=GroupType.WALLET, name="Provider Wallets", owner=provider_user
        )
    ...
    def test_wallet_group_type_validation(self, provider_user, end_user):
        ...
        network_group = GenericGroup.objects.create(
            group_type=GroupType.NETWORK, name="Networks", owner=provider_user
        )

        with pytest.raises(ValidationError) as exc_info:
            UserWalletGroup.objects.create(
                user=end_user, wallet_group=network_group, provider=provider_user
            )
    ...
    def test_default_notification_routing(self, provider_user, end_user):
        ...
        uwg = UserWalletGroup.objects.create(
            user=end_user, wallet_group=wallet_group, provider=provider_user
        )
...
class TestAlertGroupMemberValidation:
    ...
    def test_invalid_member_type_mismatch(
    ...
    def test_bulk_validation_all_valid(self, user, wallet_alert_group):
