...
def _collect_selected_catalog_ids(proposed_spec: Dict[str, Any]) -> List[str]:
    report = (
        proposed_spec.get("compile_report")
        if isinstance(proposed_spec.get("compile_report"), dict)
        else {}
    )
    ...
    exe = (
        proposed_spec.get("compiled_executable")
        if isinstance(proposed_spec.get("compiled_executable"), dict)
        else {}
    )
...
def evaluate_compiler_output(
    *,
    ...
    template = (
        proposed_spec.get("template")
        if isinstance(proposed_spec.get("template"), dict)
        else {}
    )
    ...
    if forbidden_keys & keys:
        errors.append(
            f"template contains forbidden keys: {sorted(forbidden_keys & keys)}"
        )

    if _contains_eth_address(template):
        errors.append(
            "template contains an EVM address literal (0x...40 hex); targets must not appear in templates"
        )

    selected_ids = _collect_selected_catalog_ids(proposed_spec)
    allowlisted_catalog_ids = {
        e.catalog_id for e in list_catalog_entries() if getattr(e, "enabled", True)
    }
    ...
    if not isinstance(condition_ast, dict) or not condition_ast:
        errors.append(
            "template.trigger.condition_ast must be a non-empty expression object"
        )
    elif (
        not isinstance(condition_ast.get("op"), str)
        or not str(condition_ast.get("op") or "").strip()
    ):
        errors.append(
            "template.trigger.condition_ast must include a non-empty 'op' string"
        )
    ...
    if expected_no_catalog_ids and isinstance(condition_ast, dict) and condition_ast:
        ...
        if "$.tx." not in ast_text and "$.schedule." not in ast_text:
            errors.append(
                "expected tx-only condition_ast to reference $.tx.* (or $.schedule.*), but it did not"
            )

    if expected_trigger_modes_any_of:
        expected_modes = {
            m.strip().lower()
            for m in expected_trigger_modes_any_of
            if isinstance(m, str) and m.strip()
        }
        if expected_modes and (mode_str not in expected_modes):
            errors.append(
                f"expected trigger mode in {sorted(expected_modes)}, got {mode_str!r}"
            )
    ...
    if isinstance(compile_errors, list) and [
        e for e in compile_errors if str(e).strip()
    ]:
    ...
    if expected_missing_info_codes_any_of:
        expected_missing = {
            c.strip()
            for c in expected_missing_info_codes_any_of
            if isinstance(c, str) and c.strip()
        }
        if expected_missing and not (set(missing_codes) & expected_missing):
            errors.append(
                f"expected missing_info codes to include one of {sorted(expected_missing)}, got {missing_codes}"
            )
    else:
        ...
        if missing_codes:
            ...
            scope = (
                template.get("scope") if isinstance(template.get("scope"), dict) else {}
            )
            networks = scope.get("networks")
            if not isinstance(networks, list) or not [
                n for n in networks if isinstance(n, str) and n.strip()
            ]:
                errors.append(
                    "expected template.scope.networks to be non-empty when missing_info is empty"
                )
    ...
    if expected_no_catalog_ids:
        ...
        principals = (
            signals.get("principals")
            if isinstance(signals.get("principals"), list)
            else []
        )
        factors = (
            signals.get("factors") if isinstance(signals.get("factors"), list) else []
        )
        if principals or factors:
            errors.append(
                "expected tx-only alert (no signals), but template.signals is non-empty"
            )
        compiled_executable = (
            proposed_spec.get("compiled_executable")
            if isinstance(proposed_spec.get("compiled_executable"), dict)
        ...
        if isinstance(datasources, list) and datasources:
            errors.append(
                "expected tx-only alert (no datasources), but compiled_executable.datasources is non-empty"
            )
    ...
    if expected_catalog_ids_any_of:
        ...
        if not declared_ids:
            errors.append(
                "expected at least one signal.update_sources[].ref, but none were declared"
            )
    ...
    if expected_variable_ids_all:
        variables = (
            ...
            if isinstance(template.get("variables"), list)
            else []
        )
        var_ids = {
            str(v.get("id")).strip()
            ...
            and str(v.get("id")).strip()
        }
        missing_vars = [
            vid
            for vid in expected_variable_ids_all
            if isinstance(vid, str) and vid.strip() and vid.strip() not in var_ids
        ]
        if missing_vars:
            errors.append(
                f"expected template.variables to include ids {missing_vars}, got {sorted(var_ids)}"
            )
        ...
        for var in variables:
            ...
            if var_id == "window_duration":
                ...
                if vtype != "duration":
                    errors.append(
                        f"window_duration variable must have type 'duration', got {vtype!r}"
                    )

    compiled_executable = proposed_spec.get("compiled_executable")
    if (
        not isinstance(compiled_executable, dict)
        or compiled_executable.get("schema_version") != "alert_executable_v1"
    ):
