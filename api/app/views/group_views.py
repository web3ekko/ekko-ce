...
class GenericGroupViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if self.action in {"list", "by_type", "summary"}:
            return GenericGroup.objects.filter(owner=self.request.user).select_related(
                "owner"
            )
    ...
    def list(self, request, *args, **kwargs):
        ...
        """
        my_groups = GenericGroup.objects.filter(owner=request.user).select_related(
            "owner"
        )
        public_groups = (
            GenericGroup.objects.filter(settings__visibility="public")
        ...
        for member in members_data:
            ...
            if "metadata" in member:
                ...
                if is_accounts_group:
                    merged_metadata = {
                        "owner_verified": bool(
                            merged_metadata.get("owner_verified", False)
                        ),
                        **merged_metadata,
                    }
    ...
    def by_type(self, request):
        ...
        if not group_type:
            return Response(
                {"error": "type parameter required"}, status=status.HTTP_400_BAD_REQUEST
            )
    ...
    def accounts(self, request):
        ...
        if not group:
            return Response(
                {"detail": "Accounts group not found"}, status=status.HTTP_404_NOT_FOUND
            )
    ...
    def accounts_add_wallet(self, request):
        ...
        added = accounts_group.add_member_local(
            ...
            metadata={
                "owner_verified": serializer.validated_data.get("owner_verified", False)
            },
        )
    ...
    def _parse_wallet_import_payload(
        self, fmt: str, payload: str
    ) -> List[Tuple[int, str]]:
        ...
        if fmt == "json":
            ...
            if not isinstance(parsed, list):
                raise ValidationError(
                    "JSON payload must be a list or an object with a 'wallets' list"
                )
    ...
    def _ensure_user_wallet_group_editable(self, request, uwg: UserWalletGroup) -> None:
        ...
        if not uwg.can_edit(user=request.user):
            raise PermissionDenied(
                "You do not have permission to modify this provider wallet group."
            )
    ...
    def user_wallet_groups(self, request):
        ...
        if request.method == "POST":
            ...
            serializer.is_valid(raise_exception=True)
            uwg = serializer.save()
            response_serializer = UserWalletGroupSerializer(
                uwg, context={"request": request}
            )
            return Response(response_serializer.data, status=status.HTTP_201_CREATED)

        qs = (
            UserWalletGroup.objects.filter(user=request.user)
            .select_related(
                "wallet_group",
                "provider",
                "callback",
        ...
        if page is not None:
            serializer = UserWalletGroupSerializer(
    ...
    def user_wallet_group_add_wallets(self, request, uwg_id=None):
        ...
        existing_keys = self._normalize_existing_wallet_keys(
            list(uwg.wallet_keys or [])
        )
    ...
    def user_wallet_group_import_wallets(self, request, uwg_id=None):
        ...
        if merge_mode == "replace":
            ...
            base_keys = self._normalize_existing_wallet_keys(
                list(uwg.wallet_keys or [])
            )
...
class GroupSubscriptionViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return GroupSubscription.objects.filter(owner=self.request.user).select_related(
