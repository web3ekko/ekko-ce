...
from ..services.nats_service import (
    publish_alert_created_sync,
    publish_alert_updated_sync,
    publish_alert_enabled_sync,
    publish_alert_disabled_sync,
)
...
class AlertInstanceViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        if chain_name:
            ...
            if chain_part:
                ...
                if network_part:
                    filters |= Q(
                        trigger_config__chains__contains=[
                            f"{chain_part}-{network_part}"
                        ]
                    )
        ...
        latest_only = (
            self.request.query_params.get("latest_only", "true").lower() == "true"
        )
    ...
    def create(self, request, *args, **kwargs):
        ...
        instance_name = (
            str(data.get("name") or "").strip() or str(template.name).strip()
        )
        ...
        response_serializer = AlertInstanceSerializer(
    ...
    def versions(self, request, pk=None):
        ...
        versions = AlertInstance.objects.filter(id=alert_instance.id).order_by(
            "-version"
        )
    ...
    def version(self, request, pk=None):
        ...
        try:
            alert_instance = AlertInstance.objects.get(
                id=alert_id, version=int(version), user=request.user
            )
            ...
            return Response(
                {
                    ...
                    "estimated_wait_ms": int(
                        getattr(settings, "NLP_ESTIMATED_WAIT_MS", 2000)
                    ),
                },
    ...
    def inline_preview(self, request):
        ...
        from ..services.preview import PreviewDataFetcher, SimpleDjangoEvaluator
        ...
        if not spec:
            return Response(
                {"error": "spec is required"}, status=status.HTTP_400_BAD_REQUEST
            )
...
class ChainViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def sub_chains(self, request, pk=None):
        ...
        sub_chains = SubChain.objects.filter(chain=chain, enabled=True).order_by(
            "display_name"
        )
...
class TeamNotificationChannelEndpointViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        ...
        user_teams = TeamMember.objects.filter(user=self.request.user).values_list(
            "team_id", flat=True
        )
        return NotificationChannelEndpoint.objects.filter(
            owner_type="team", owner_id__in=user_teams
        ).order_by("-created_at")
    ...
    def create(self, request, *args, **kwargs):
        ...
        # Check permissions
        if not self.check_team_permissions(
            team_id, [TeamMemberRole.OWNER, TeamMemberRole.ADMIN]
        ):
            return Response(
    ...
    def update(self, request, *args, **kwargs):
        ...
        if not self.check_team_permissions(
            endpoint.owner_id, [TeamMemberRole.OWNER, TeamMemberRole.ADMIN]
        ):
            return Response(
...
class TeamMemberNotificationOverrideViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response(
                {"error": "Override not found"}, status=status.HTTP_404_NOT_FOUND
            )
