...
class DashboardStatsView(APIView):
    ...
    def get(self, request):
        ...
        executions_24h = AlertExecution.objects.filter(
            alert_instance__user=user, created_at__gte=last_24h
        ).count()

        executions_7d = AlertExecution.objects.filter(
            alert_instance__user=user, created_at__gte=last_7d
        ).count()

        # Triggered alerts in last 24h
        triggered_24h = AlertExecution.objects.filter(
            alert_instance__user=user, created_at__gte=last_24h, result=True
        ).count()
...
class DashboardActivityView(APIView):
    ...
    def get(self, request):
        ...
        if not activity_type or activity_type == "execution":
            executions = (
                AlertExecution.objects.filter(alert_instance__user=user)
                .select_related("alert_instance")
        ...
        if not activity_type or activity_type == "alert_created":
            recent_alerts = AlertInstance.objects.filter(user=user).order_by(
                "-created_at"
            )[:limit]
...
class DashboardChainStatsView(APIView):
    ...
    def get(self, request):
        ...
        if chains_param:
            chain_filter = [
                c.strip().lower() for c in chains_param.split(",") if c.strip()
            ]
        ...
        for alert in alerts:
            ...
            for chain in chains:
                ...
                if isinstance(chain, dict):
                    chain_name = (
                        chain.get("chain_id") or chain.get("network") or "unknown"
                    )
                else:
        ...
        # Build response
        chains = []
        for chain_key, stat in sorted(
            chain_stats.items(), key=lambda x: -x[1]["total"]
        ):
            chains.append(
                {
                    "chain": stat["chain"],
...
class DashboardNetworkStatusView(APIView):
    ...
    def get(self, request):
        ...
        for provider in provider_statuses:
            for chain_id, sub in provider.get("subscriptions", {}).items():
                networks.append(self._transform_subscription(provider, chain_id, sub))
    ...
    def _calculate_health_score(self, sub: dict) -> int:
        ...
        metrics = sub.get("metrics", {})
        errors = metrics.get("connection_errors", 0) + metrics.get(
            "processing_errors", 0
        )
