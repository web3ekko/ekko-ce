...
def _require_admin(user, team_id):
    ...
    if not membership:
        return None, Response(
            {"error": "Not a member of this team"}, status=status.HTTP_403_FORBIDDEN
        )
    if membership.role not in [TeamMemberRole.OWNER, TeamMemberRole.ADMIN]:
        return None, Response(
            {"error": "Admin permissions required"}, status=status.HTTP_403_FORBIDDEN
        )
...
class TeamListView(APIView):
    ...
    def get(self, request):
        teams = (
            Team.objects.filter(members__user=request.user, members__is_active=True)
...
class TeamMembersView(APIView):
    ...
    def get(self, request, team_id):
        ...
        members = (
            TeamMember.objects.filter(team_id=team_id)
            .select_related("user")
            .order_by("-created_at")
        )
        results = []
            ...
            results.append(
                {
                    ...
                    "last_active_at": user.last_login.isoformat()
                    if user.last_login
                    else None,
                }
            )
...
class TeamInviteView(APIView):
    ...
    def post(self, request, team_id):
        ...
        membership = TeamMember.objects.filter(
            team_id=team_id, user=invited_user
        ).first()
        if membership:
            return Response(
        ...
        return Response(
            {
                "member": {
                    ...
                    "last_active_at": invited_user.last_login.isoformat()
                    if invited_user.last_login
                    else None,
                }
            },
...
class TeamMemberDetailView(APIView):
    ...
    def patch(self, request, team_id, member_id):
        ...
        member = (
            TeamMember.objects.filter(team_id=team_id, id=member_id)
            .select_related("user")
            .first()
        )
        if not member:
        ...
        return Response(
            {
                ...
                "status": "active" if member.is_active else "pending",
                "joined_at": member.joined_at.isoformat(),
                "last_active_at": member.user.last_login.isoformat()
