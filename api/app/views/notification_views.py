...
from app.serializers.notification_serializers import (
    ...
    GroupNotificationSettingsSerializer,
...
class UserNotificationSettingsViewSet(ModelViewSet):
    ...
    def test_notification(self, request):
        ...
        logger.info(
            f"Notification settings validated for user {request.user.id} via {channel}"
        )
...
class NotificationChannelEndpointViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            NotificationChannelEndpoint.objects.filter(
                owner_type="user", owner_id=self.request.user.id
            )
    ...
    def perform_destroy(self, instance):
        ...
        logger.info(f"Deleted {channel_type} endpoint '{label}' for user {user_id}")
    ...
    def request_verification(self, request, pk=None):
        ...
        if endpoint.verified and endpoint.enabled:
            return Response(
                {"message": "Endpoint is already verified"}, status=status.HTTP_200_OK
            )
        ...
        if endpoint.channel_type == "telegram":
            ...
            if chat_id:
                cache_service.store_verification_code(
                    str(request.user.id), chat_id, code
                )
        ...
        return Response(
            {
                ...
                "verification_code": code
                if endpoint.channel_type == "telegram"
                else None,  # Return code for Telegram testing
    ...
    def verify(self, request, pk=None):
        ...
        try:
            verification = NotificationChannelVerification.objects.get(
                endpoint=endpoint, verification_code=code, verified_at__isnull=True
            )
    ...
    def resend_verification(self, request, pk=None):
        ...
        NotificationChannelVerification.objects.filter(
            endpoint=endpoint, verified_at__isnull=True
    ...
    def test(self, request, pk=None):
        ...
        if not endpoint.enabled:
            return Response(
                {"error": "Endpoint is disabled"}, status=status.HTTP_400_BAD_REQUEST
            )

        # Get optional test message from request
        test_message = request.data.get(
            "message", "Test notification from Ekko Dashboard"
        )
        ...
        if endpoint.channel_type in ["slack", "telegram"]:
            ...
            try:
                ...
                async def send_test_notification():
                    ...
                    await nc.publish(subject, json.dumps(notification_payload).encode())
                ...
                return Response(
                    {
                        ...
                        "avg_response_time_ms": health_metrics.get(
                        ...
                        "consecutive_failures": health_metrics.get(
...
class TeamNotificationChannelEndpointViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        ...
        return (
            NotificationChannelEndpoint.objects.filter(
                owner_type="team", owner_id__in=user_team_ids
            )
            .select_related("created_by")
            .order_by("-created_at")
        )
    ...
    def _is_team_admin(self, team_id):
        ...
        try:
            membership = TeamMember.objects.get(
                user=self.request.user, team_id=team_id, is_active=True
            )
    ...
    def perform_create(self, serializer):
        ...
        print(
            f"XYZABC_TEAM_PERFORM_CREATE: team_id={team_id}",
            file=sys.stderr,
            flush=True,
        )
...
class TeamMemberNotificationOverrideViewSet(ModelViewSet):
    ...
    def get_object(self):
        ...
        try:
            TeamMember.objects.get(
                team_id=team_id, user=self.request.user, is_active=True
            )
        except TeamMember.DoesNotExist:
    ...
    def create(self, request):
        ...
        # Verify user is a member of the team
        try:
            TeamMember.objects.get(team_id=team_id, user=request.user, is_active=True)
        except TeamMember.DoesNotExist:
            return Response(
    ...
    def destroy(self, request, pk=None):
        ...
        try:
            ...
            return Response({"message": "Notification override reset to defaults"})
        except ValueError as e:
            return Response({"error": str(e)}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def disable_all_team_notifications(self, request):
        ...
        return Response(
            {"message": "All team notifications disabled", "team_id": str(team_id)}
        )
    ...
    def enable_all_team_notifications(self, request):
        ...
        return Response(
            {"message": "All team notifications enabled", "team_id": str(team_id)}
        )
    ...
    def _invalidate_member_override_cache(self, user_id, team_id):
        ...
        cache.delete(cache_key)
        logger.debug(f"Invalidated override cache for user {user_id} in team {team_id}")
...
class GroupNotificationSettingsViewSet(ModelViewSet):
    ...
    def test_group_notification(self, request, pk=None):
        ...
        logger.info(
            f"Group notification settings validated for group {group_settings.group.id} via {channel}"
        )
...
class NotificationDeliveryViewSet(ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        queryset = (
            NotificationDelivery.objects.filter(user=self.request.user)
    ...
    def statistics(self, request):
        ...
        channel_stats = (
            ...
            .annotate(
                count=Count("id"),
                success_rate=Count("id", filter=Q(status="delivered"))
                * 100.0
                / Count("id"),
            )
            .order_by("-count")
...
class NotificationTemplateViewSet(ModelViewSet):
    ...
    def channels(self, request):
        ...
        channel_stats = (
            NotificationTemplate.objects.filter(active=True)
            .values("channel")
...
class NotificationHealthAPIView(APIView):
    ...
    def get(self, request):
        ...
        health_status = {"status": "healthy", "timestamp": timezone.now(), "checks": {}}
    ...
    def get(self, request):
        ...
        try:
            ...
            async def run_query() -> DeliveryMetrics:
                ...
                try:
                    return await client.get_platform_metrics(
                        start_date, end_date, channel_type
                    )
                finally:
                    await client.close()
