...
class GlobalSearchView(APIView):
    ...
    def get(self, request):
        ...
        if types_param:
            types = [
                t.strip() for t in types_param.split(",") if t.strip() in allowed_types
            ]
        else:
            types = allowed_types
        ...
        if "alert" in types:
            alerts = AlertInstance.objects.filter(
                Q(name__icontains=query) | Q(nl_description__icontains=query), user=user
            )[:limit]
                ...
                results.append(
                    {
                        ...
                        "type": "alert",
                        "title": alert.name,
                        "subtitle": (alert.nl_description or "")[:100]
                        if alert.nl_description
                        else None,
                        "url": f"/dashboard/alerts/{alert.id}",
                        "icon": "bell",
                        "metadata": {
                            "enabled": alert.enabled,
                            "chains": alert.get_chains()
                            if hasattr(alert, "get_chains")
                            else [],
                        },
                    }
                )
                ...
                results.append(
                    {
                        ...
                        "type": "template",
                        "title": template.name,
                        "subtitle": template.description[:100]
    ...
    def _get_matching_pages(self, query, limit):
        ...
        pages = [
            ...
            },
            {
                "id": "marketplace",
                "title": "Marketplace",
                "subtitle": "Discover alert templates",
            ...
            },
            {
                "id": "api",
                "title": "Developer API",
                "subtitle": "API documentation and keys",
                "url": "/dashboard/api",
                "icon": "code",
            },
            {
        ...
        actions = [
            {
                ...
                "subtitle": "Set up a new blockchain alert",
                "icon": "plus",
                "metadata": {"action": "create-alert"},
            },
            {
                "id": "add-wallet",
                "title": "Add Wallet",
                "subtitle": "Connect a new wallet",
                "icon": "plus",
                "metadata": {"action": "add-wallet"},
            },
            {
                "id": "export-data",
                "title": "Export Data",
...
class SearchWalletsView(APIView):
    ...
    def get(self, request):
        ...
        accounts_members = (
            accounts_group.member_data.get("members", {}) if accounts_group else {}
        ) or {}
        ...
        if networks:
            ...
            chain_by_network = {
                str(c.native_token or "").upper(): c for c in chains if c.native_token
            }
        ...
        for wallet in wallets:
            ...
            if chain:
                chain_id = subnet_chain_id.get((network, subnet)) or getattr(
                    chain, "chain_id", None
                )

            nickname = None
            if chain_id is not None:
                nickname = nicknames_by_address_chain.get(
                    f"{address.lower()}:{int(chain_id)}"
                )

            title = accounts_label or nickname or _truncate_middle(address)
            subtitle = wallet_key
            url = (
                f"/dashboard/wallets/{quote(wallet_key, safe='')}"
                if in_accounts
                else "/dashboard/wallets"
            )
...
class SearchSuggestionsView(APIView):
    ...
    def get(self, request):
        ...
        alert_names = (
            AlertInstance.objects.filter(user=user, name__icontains=query)
            .values_list("name", flat=True)
            .distinct()[:limit]
        )

        for name in alert_names:
            suggestions.append({"text": name, "type": "alert", "icon": "bell"})

        # Get template suggestions
        template_names = (
            AlertTemplate.objects.filter(
                Q(is_public=True) | Q(created_by=user), name__icontains=query
            )
            .values_list("name", flat=True)
            .distinct()[:limit]
        )
        ...
        for name in template_names:
            suggestions.append({"text": name, "type": "template", "icon": "chart"})
        ...
        return Response({"query": query, "suggestions": unique_suggestions})
