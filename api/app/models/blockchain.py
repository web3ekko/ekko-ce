...
class BlockchainNode(models.Model):
    ...
    )  # Changed to CharField to accept ws:// and wss:// URLs
    ...
    def clean(self):
        ...
        if self.is_primary and self.enabled:
            existing_primary = BlockchainNode.objects.filter(
                network=self.network, subnet=self.subnet, is_primary=True, enabled=True
            ).exclude(pk=self.pk)
        ...
        if self.ws_url and not (
            self.ws_url.startswith("ws://") or self.ws_url.startswith("wss://")
        ):
            raise ValidationError(
