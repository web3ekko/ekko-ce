...
class NotificationChannelPreferences(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="channel_preferences"
    )
    ...
    enabled = models.BooleanField(
        default=True, help_text="Whether notifications are enabled for this channel"
    )
    ...
    def warm_cache(self):
        ...
        all_prefs = NotificationChannelPreferences.objects.filter(user_id=self.user_id)
...
def warm_channel_preferences_cache_on_delete(sender, instance, **kwargs):
    ...
    if remaining_prefs.exists():
        ...
        cache.set(cache_key, cache_data, timeout=3600)
        logger.info(
            f"Re-warmed channel preferences cache for user {instance.user_id} (preference deleted)"
        )
    else:
        # No preferences left - delete cache entry
        cache.delete(cache_key)
        logger.info(
            f"Deleted channel preferences cache for user {instance.user_id} (last preference deleted)"
        )
...
class UserNotificationSettings(models.Model):
    ...
    channels = models.JSONField(
        default=dict, help_text="Channel-specific settings (email, slack, sms, etc.)"
    )
        ...
        for channel_name, settings in self.channels.items():
            ...
            if not isinstance(settings, dict):
                raise ValidationError(
                    f"Channel settings must be a dictionary for {channel_name}"
                )
    ...
    def is_in_quiet_hours(self) -> bool:
        ...
        try:
            ...
            start_time = datetime.strptime(
                self.quiet_hours["start_time"], "%H:%M"
            ).time()
    ...
    def create_default_settings(cls, user: User) -> "UserNotificationSettings":
        ...
        default_channels = {"websocket": {"enabled": True, "config": {}}}
...
class GroupNotificationSettings(models.Model):
    ...
    group = models.ForeignKey(
        Team, on_delete=models.CASCADE, related_name="notification_settings"
    )
    ...
    shared_channels = models.JSONField(
        default=dict, help_text="Shared notification channels for the entire group"
    )

    # Whether members can override group settings
    member_overrides_allowed = models.BooleanField(
        default=True, help_text="Allow group members to override certain settings"
    )
...
class NotificationDelivery(models.Model):
    ...
    # Delivery details
    channel = models.CharField(
        max_length=50, help_text="Notification channel (email, slack, sms, etc.)"
    )
    ...
    provider_response = models.JSONField(
        null=True, blank=True, help_text="Full response from external provider"
    )
    ...
    def mark_failed(
        self, error_code: str = None, error_message: str = None, retryable: bool = True
    ):
...
class NotificationTemplate(models.Model):
    ...
    name = models.CharField(
        max_length=100, unique=True, help_text="Unique template identifier"
    )
    ...
    subject_template = models.CharField(
        max_length=200, help_text="Subject line template (email/notifications)"
    )

    text_template = models.TextField(help_text="Plain text template")

    html_template = models.TextField(
        null=True, blank=True, help_text="HTML template for rich content channels"
    )
...
class NotificationChannelEndpoint(models.Model):
    ...
    verified_at = models.DateTimeField(
        null=True, blank=True, help_text="Timestamp of last verification"
    )
    ...
    def clean(self):
        ...
        if self.channel_type == "email":
            ...
            try:
                ...
                raise ValidationError(
                    f"Invalid email address: {self.config['address']}"
                )
            ...
            if "webhook_url" not in self.config:
                raise ValidationError(
                    "Discord channel requires 'webhook_url' in config"
                )
            ...
            if "phone_number" not in self.config:
                raise ValidationError(
                    "WhatsApp channel requires 'phone_number' in config"
                )
    ...
    def warm_cache(self):
        ...
        cache.set(cache_key, cache_data, timeout=3600)
        logger.info(
            f"Warmed notification endpoint cache for {self.owner_type}:{self.owner_id}"
        )
...
class TeamMemberNotificationOverride(models.Model):
    ...
    def invalidate_cache(self):
        ...
        cache_key = (
            f"notification:overrides:team:{self.team.id}:member:{self.member.id}"
        )
        cache.delete(cache_key)
        logger.info(
            f"Invalidated notification override cache for team {self.team.id}, member {self.member.id}"
        )
...
class NotificationChannelVerification(models.Model):
    ...
    verification_code = models.CharField(
        max_length=6, help_text="6-digit verification code"
    )
    ...
    verified_at = models.DateTimeField(
        null=True, blank=True, help_text="Timestamp when verification was completed"
    )
...
class NotificationCache:
    ...
    def warm_user_cache(user_ids: List[int]) -> int:
        ...
        logger.info(
            f"Warmed notification cache for {cached_count}/{len(user_ids)} users"
        )
        return cached_count
    ...
    def clear_all_user_caches():
        ...
        logger.warning(
            "Clearing all user notification caches - implement with Redis SCAN"
        )
...
def invalidate_user_settings_cache_on_delete(sender, instance, **kwargs):
    ...
    try:
        ...
        logger.info(
            f"Invalidated notification settings cache for user {instance.user_id} (deleted)"
        )
    except Exception as e:
        # Signal handlers should NOT raise exceptions that break model operations
        logger.error(
            f"Error invalidating notification settings cache for user {instance.user_id}: {e}"
        )
