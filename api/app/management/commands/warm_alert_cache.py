...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Projected {len(catalog_entries)} datasource catalog entries"
            )
        )
        ...
        for tmpl_ver in AlertTemplateVersion.objects.select_related("template").all():
            projected = projector.project_template_bundle(
                template_id=str(tmpl_ver.template_id),
                template_version=int(tmpl_ver.template_version),
            )
            if projected is not None:
                bundle_count += 1
        self.stdout.write(
            self.style.SUCCESS(f"Projected {bundle_count} template bundles")
        )
