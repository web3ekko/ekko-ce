...
class Command(BaseCommand):
    ...
    BLOCKCHAINS = [
        {
            ...
            "symbol": "eth",
            "chain_type": "EVM",
            "subnets": ["mainnet", "sepolia", "goerli"],
        },
        {
            "name": "Polygon",
            "symbol": "matic",
            "chain_type": "EVM",
            "subnets": ["mainnet", "mumbai"],
        },
        {
            "name": "Arbitrum",
            "symbol": "arb",
            "chain_type": "EVM",
            "subnets": ["mainnet", "goerli"],
        },
        ...
        },
        {
            ...
            "symbol": "btc",
            "chain_type": "Bitcoin",
            "subnets": ["mainnet", "testnet"],
        },
        {
            "name": "Solana",
            "symbol": "sol",
            "chain_type": "Solana",
            "subnets": ["mainnet-beta", "devnet"],
        },
    ]
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            ...
            for user in users:
                user_wallets = (
                    [w for w in all_wallets if w.created_by_id == user.id]
                    if hasattr(Wallet, "created_by")
                    else all_wallets[:wallets_per_user]
                )
    ...
    def _clear_test_data(self, verbose):
        ...
        deleted_wallets = Wallet.objects.filter(name__in=self.WALLET_NAMES).delete()[0]

        # Clear test users
        deleted_users = User.objects.filter(email__endswith="@test.ekko.zone").delete()[
            0
        ]
    ...
    def _create_alert_instances(
        self, AlertInstance, user, templates, wallets, count, verbose
    ):
        ...
        for i in range(count):
            ...
            trigger_config = self._generate_trigger_config(
                trigger_type, cron_expressions
            )
    ...
    def _print_summary(self, users, wallets, templates, alerts_per_user):
        ...
        self.stdout.write("")
        self.stdout.write(
            self.style.HTTP_INFO("  💡 Run E2E tests with: pytest tests/e2e/ -v")
        )
