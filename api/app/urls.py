...
from .views.notification_views import (
    ...
    GroupNotificationSettingsViewSet,
    NotificationDeliveryViewSet,
    NotificationTemplateViewSet,
    BulkNotificationAPIView,
    NotificationCacheAPIView,
    NotificationHealthAPIView,
    PlatformHealthMetricsAPIView,
    ChannelHealthMetricsAPIView,
...
router.register(
    r"alerts/default-network",
    DefaultNetworkAlertViewSet,
    basename="default-network-alert",
)
...
router.register(
    r"notifications/channels",
    NotificationChannelEndpointViewSet,
    basename="notification-channels",
)
router.register(
    r"team-notification-endpoints",
    TeamNotificationChannelEndpointViewSet,
    basename="team-notification-endpoints",
)
router.register(
    r"team-notification-overrides",
    TeamMemberNotificationOverrideViewSet,
    basename="team-notification-overrides",
)
router.register(
    r"group-notification-settings",
    GroupNotificationSettingsViewSet,
    basename="group-notification-settings",
)
router.register(
    r"notification-deliveries",
    NotificationDeliveryViewSet,
    basename="notification-deliveries",
)
router.register(
...
urlpatterns = [
    ...
    path(
        "alert-jobs/by-trigger-type/",
        GetActiveAlertsByTriggerTypeView.as_view(),
        name="alerts-by-trigger-type",
    ),
    path(
        "alert-jobs/event-driven/",
        GetMatchingEventDrivenAlertsView.as_view(),
        name="event-driven-alerts",
    ),
    path(
        "alert-jobs/record-job/",
        RecordJobCreationView.as_view(),
        name="record-job-creation",
    ),
    ...
    path(
        "notifications/bulk/",
        BulkNotificationAPIView.as_view(),
        name="bulk-notifications",
    ),
    path(
        "notifications/cache/",
        NotificationCacheAPIView.as_view(),
        name="notification-cache",
    ),
    path(
        "notifications/health/",
        NotificationHealthAPIView.as_view(),
        name="notification-health",
    ),
    path(
        "notifications/platform-metrics/",
        PlatformHealthMetricsAPIView.as_view(),
        name="platform-health-metrics",
    ),
    path(
        "notifications/channel-metrics/<str:channel_id>/",
        ChannelHealthMetricsAPIView.as_view(),
        name="channel-health-metrics",
    ),
    path(
        "notifications/history/",
        UserNotificationHistoryAPIView.as_view(),
        name="notification-history",
    ),
    # Profile API (v1)
    path("v1/profile/", ProfileView.as_view(), name="profile"),
    path(
        "v1/profile/preferences/", PreferencesView.as_view(), name="profile-preferences"
    ),
    path("v1/profile/avatar/", AvatarView.as_view(), name="profile-avatar"),
    path(
        "v1/profile/connected-services/",
        ConnectedServicesView.as_view(),
        name="profile-connected-services",
    ),
    path(
        "v1/profile/connected-services/<uuid:service_id>/",
        ConnectedServiceDetailView.as_view(),
        name="profile-connected-service-detail",
    ),
    path("v1/profile/sessions/", SessionsView.as_view(), name="profile-sessions"),
    path(
        "v1/profile/sessions/revoke-all/",
        RevokeAllSessionsView.as_view(),
        name="profile-revoke-all-sessions",
    ),
    path(
        "v1/profile/sessions/<str:session_id>/",
    ...
    path(
        "v1/teams/<uuid:team_id>/members/",
        TeamMembersView.as_view(),
        name="team-members",
    ),
    path(
        "v1/teams/<uuid:team_id>/invite/", TeamInviteView.as_view(), name="team-invite"
    ),
    path(
        "v1/teams/<uuid:team_id>/members/<uuid:member_id>/",
        TeamMemberDetailView.as_view(),
        name="team-member-detail",
    ),
    path(
        "v1/teams/<uuid:team_id>/members/<uuid:member_id>/resend-invite/",
        TeamMemberResendInviteView.as_view(),
        name="team-member-resend-invite",
    ),
    # Search API (v1)
    ...
    path(
        ...
        SearchTransactionsView.as_view(),
    ...
    # Billing API (v1)
    path(
        "v1/billing/overview/", BillingOverviewView.as_view(), name="billing-overview"
    ),
    path(
    ...
    path(
        "v1/developer/api-keys/<uuid:key_id>/",
        ApiKeyDetailView.as_view(),
        name="developer-api-key-detail",
    ),
    path(
        "v1/developer/api-keys/<uuid:key_id>/revoke/",
        ApiKeyRevokeView.as_view(),
        name="developer-api-key-revoke",
    ),
    path("v1/developer/usage/", ApiUsageView.as_view(), name="developer-usage"),
    path(
        "v1/developer/endpoints/",
        ApiEndpointsView.as_view(),
    ...
    path(
        "v1/dashboard/chain-stats/",
        DashboardChainStatsView.as_view(),
        name="dashboard-chain-stats",
    ),
    path(
        "v1/dashboard/network-status/",
        DashboardNetworkStatusView.as_view(),
        name="dashboard-network-status",
    ),
    ...
    path(
        "v1/analytics/tables/<str:table_name>/schema/",
        analytics_table_schema,
        name="analytics-table-schema",
    ),
    path(
        "v1/analytics/wallet/<str:address>/transactions/",
        wallet_transactions,
        name="analytics-wallet-transactions",
    ),
    path(
        "v1/analytics/wallet/<str:address>/transfers/",
        wallet_token_transfers,
        name="analytics-wallet-transfers",
    ),
    path(
        "v1/analytics/wallet/<str:address>/balances/",
        wallet_balances,
        name="analytics-wallet-balances",
    ),
    path(
        "v1/analytics/block/<int:block_number>/",
        block_info,
        name="analytics-block-info",
    ),
    path(
        "v1/analytics/token/<str:token_address>/prices/",
        token_prices,
        name="analytics-token-prices",
    ),
    # Newsfeed API (v1) - Transaction feed for monitored wallets
    path(
        "v1/analytics/newsfeed/transactions/",
        newsfeed_transactions,
        name="analytics-newsfeed-transactions",
    ),
]
...
if settings.DEBUG:
    ...
    urlpatterns += [
        path(
            ...
            test_views.get_verification_code,
            name="test-verification-code",
        ),
