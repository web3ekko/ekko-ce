...
class OrganizationFactory(factory.django.DjangoModelFactory):
    ...
    slug = factory.LazyAttribute(
        lambda obj: f"{slugify(obj.name)}-{uuid.uuid4().hex[:8]}"
    )  # Unique slug
...
def create_organization_with_teams(team_count=3, **org_kwargs):
    ...
    for i in range(team_count):
        team = TeamFactory(
            organization=organization, name=f"Team {i+1}", slug=f"team-{i+1}"
        )
        teams.append(team)
...
def create_team_with_members(member_count=5, **team_kwargs):
    ...
    # Create team owner
    owner = TeamMemberFactory(team=team, role=TeamMemberRole.OWNER, invited_by=None)

    # Create other members
    members = [owner]
    for i in range(member_count - 1):
        role = fake.random_element(
            [TeamMemberRole.ADMIN, TeamMemberRole.MEMBER, TeamMemberRole.VIEWER]
        )
        member = TeamMemberFactory(team=team, role=role, invited_by=owner.user)
...
def create_user_with_organization_membership(role=TeamMemberRole.MEMBER):
    ...
    membership = TeamMemberFactory(team=team, user=user, role=role)
...
def create_organization_hierarchy():
    ...
    # Engineering teams
    eng_team = TeamFactory(organization=org, name="Engineering", slug="engineering")

    frontend_team = TeamFactory(organization=org, name="Frontend", slug="frontend")

    backend_team = TeamFactory(organization=org, name="Backend", slug="backend")

    # Product team
    product_team = TeamFactory(organization=org, name="Product", slug="product")
    ...
    for team in teams:
        # Team lead (owner)
        lead = TeamMemberFactory(team=team, role=TeamMemberRole.OWNER, invited_by=None)

        # Senior members (admins)
        seniors = [
            TeamMemberFactory(
                team=team, role=TeamMemberRole.ADMIN, invited_by=lead.user
            )
            for _ in range(2)
        ]
        ...
        members = [
            TeamMemberFactory(
                team=team, role=TeamMemberRole.MEMBER, invited_by=lead.user
            )
