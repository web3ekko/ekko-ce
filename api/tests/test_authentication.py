...
class PasswordlessAuthenticationFlowTest(TestCase):
    ...
    def test_signup_flow_automatic_knox_token_generation(self, mock_send_email):
        ...
        response = self.client.post("/api/auth/signup/", {"email": self.test_email})
        ...
        response = self.client.post(
            ...
            {"email": self.test_email, "code": code.code},
        )
    ...
    def test_authentication_performance_target(self, mock_send_email):
        ...
        user = User.objects.create(email=self.test_email, is_email_verified=True)

        # Request signin code
        response = self.client.post(
            "/api/auth/signin/email/send-code/", {"email": self.test_email}
        )
        self.assertEqual(response.status_code, 200)
...
class UserRegistrationProcessTest(TestCase):
    ...
    @patch("authentication.views.send_verification_code_email")
    def test_automatic_knox_token_generation_no_separate_api_call(
...
class KnoxTokenManagementTest(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create(email="test@ekko.zone", is_email_verified=True)
    ...
    def test_validate_token_endpoint_with_valid_token(self):
        ...
        client = Client()
        response = client.get(
            "/api/auth/validate-token/", HTTP_AUTHORIZATION=f"Token {token_key}"
        )
    ...
    def test_validate_token_endpoint_with_invalid_token(self):
        ...
        client = Client()
        response = client.get(
            "/api/auth/validate-token/", HTTP_AUTHORIZATION="Token invalid_token_12345"
        )
...
class AuthenticationErrorHandlingTest(TestCase):
    ...
    def test_invalid_email_validation(self):
        ...
        response = self.client.post(
            "/api/auth/signup/", {"email": "invalid-email-format"}
        )
    ...
    def test_knox_token_automatic_refresh(self):
        ...
        user = User.objects.create(email="test@ekko.zone", is_email_verified=True)
...
class SecurityRequirementsTest(TestCase):
    ...
    def test_rate_limiting_configuration(self):
        ...
        throttle_classes = getattr(settings, "REST_FRAMEWORK", {}).get(
...
class IntegrationTest(TestCase):
    ...
    @patch("authentication.views.send_verification_code_email")
    def test_complete_authentication_flow_specification_compliance(
        self, mock_send_email
    ):
        ...
        response = self.client.post("/api/auth/signup/", {"email": test_email})
        self.assertEqual(response.status_code, 200)
        ...
        response = self.client.post(
            "/api/auth/signup/verify-code/", {"email": test_email, "code": code.code}
        )
