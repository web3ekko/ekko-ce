...
class TestGenericGroupViewSet:
    ...
    def test_list_groups_excludes_public_groups(self):
        ...
        public_group = WalletGroupFactory(
            owner=self.other_user, settings={"visibility": "public"}
        )
    ...
    def test_list_public_groups_includes_public_groups_owned_by_others(self):
        ...
        own_public_group = WalletGroupFactory(
            owner=self.user, settings={"visibility": "public"}
        )
        public_group = WalletGroupFactory(
            owner=self.other_user, settings={"visibility": "public"}
        )
    ...
    def test_create_group_minimal(self):
        ...
        url = reverse("alerts:groups-list")
        data = {"group_type": GroupType.WALLET, "name": "My Wallets"}
    ...
    def test_create_group_with_initial_members(self):
        ...
        data = {
            ...
            "initial_members": [
                {
                    "key": "eth:MainNet:0xAbCdEf000000000000000000000000000000000000",
                    "label": "Treasury",
                },
                {
                    "key": "ETH:mainnet:0x0987654321098765432109876543210987654321",
                    "label": "Hot Wallet",
                    "tags": ["active"],
                },
            ],
    ...
    def test_add_members(self):
        ...
        data = {
            "members": [
                {
                    "member_key": "ETH:mainnet:0x1234567890123456789012345678901234567890",
                    "label": "Wallet 1",
                },
                {
    ...
    def test_add_members_alert_group_requires_templates(self):
        ...
        alert_group = AlertGroupFactory(
            owner=self.user, settings={"alert_type": "wallet"}
        )
    ...
    def test_remove_members(self):
        ...
        data = {"members": [{"member_key": keys[0]}]}
        response = self.client.post(url, data, format="json")
    ...
    def test_remove_nonexistent_members(self):
        ...
        url = reverse("alerts:groups-remove-members", args=[group.id])
        data = {"members": [{"member_key": "ETH:mainnet:0xnonexistent"}]}
        response = self.client.post(url, data, format="json")
    ...
    def test_summary_action(self):
        ...
        WalletGroupFactory(owner=self.user)
        create_group_with_members(
            owner=self.user, group_type=GroupType.WALLET, member_count=5
        )
        AlertGroupFactory(owner=self.user)
...
class TestAlertGroupTemplatesEndpoint:
    ...
    def test_templates_endpoint_lists_group_templates(self):
        alert_group = AlertGroupFactory(
            owner=self.user, settings={"alert_type": "network"}
        )
        template = AlertTemplateFactory(
            created_by=self.user, event_type="PROTOCOL_EVENT"
        )
...
class TestGroupSubscriptionViewSet:
    ...
    def test_list_subscriptions_returns_user_subscriptions_only(self):
        ...
        user_subscription = GroupSubscriptionFactory(
            alert_group=alert_group, target_group=target_group, owner=self.user
        )

        other_alert = AlertGroupFactory(owner=self.other_user)
        other_target = WalletGroupFactory(owner=self.other_user)
        GroupSubscriptionFactory(
            alert_group=other_alert, target_group=other_target, owner=self.other_user
        )
    ...
    def test_create_subscription_requires_template_params_when_group_has_templates(
        self,
    ):
    ...
    def test_create_subscription_does_not_require_network_chain_subnet_template_params(
        self,
    ):
    ...
    def test_create_subscription_token_alert_group_requires_token_target_group(self):
        ...
        alert_group = AlertGroupFactory(
            owner=self.user, settings={"alert_type": "token"}
        )
        template = AlertTemplateFactory(
            created_by=self.user,
        ...
        token_target_group = GenericGroupFactory(
            owner=self.user, group_type=GroupType.TOKEN
        )
        response = self.client.post(
            url,
            {
    ...
    def test_create_subscription_protocol_alert_group_requires_protocol_target_group(
        self,
    ):
        """Protocol-type AlertGroups can only target protocol groups or protocol keys."""
        alert_group = AlertGroupFactory(
            owner=self.user, settings={"alert_type": "protocol"}
        )
        template = AlertTemplateFactory(
            created_by=self.user,
        ...
        protocol_target_group = GenericGroupFactory(
            owner=self.user, group_type=GroupType.PROTOCOL
        )
        response = self.client.post(
            url,
            {
    ...
    def test_create_subscription_with_target_key_materializes_enabled_alert_instances(
        self,
    ):
        ...
        assert instance.target_keys == [
            "ETH:mainnet:0xabcdef000000000000000000000000000000000000"
        ]

    def test_create_subscription_with_protocol_target_key_materializes_enabled_alert_instances(
        self,
    ):
    ...
    def test_create_subscription_same_group_fails(self):
        ...
        url = reverse("alerts:subscriptions-list")
        data = {"alert_group": str(alert_group.id), "target_group": str(alert_group.id)}
        response = self.client.post(url, data, format="json")
    ...
    def test_create_subscription_can_target_public_group_owned_by_other_user(self):
        ...
        alert_group = AlertGroupFactory(owner=self.user)
        public_target_group = WalletGroupFactory(
            owner=self.other_user, settings={"visibility": "public"}
        )
        ...
        response = self.client.post(
            ...
            {
                "alert_group": str(alert_group.id),
                "target_group": str(public_target_group.id),
            },
            format="json",
        )
    ...
    def test_create_subscription_rejects_private_group_owned_by_other_user(self):
        ...
        alert_group = AlertGroupFactory(owner=self.user)
        private_target_group = WalletGroupFactory(
        ...
        response = self.client.post(
            ...
            {
                "alert_group": str(alert_group.id),
                "target_group": str(private_target_group.id),
            },
            format="json",
        )
    ...
    def test_create_subscription_to_public_alert_group_owned_by_other_user(self):
        ...
        public_alert_group = AlertGroupFactory(
            owner=self.other_user,
            settings={"alert_type": "wallet", "visibility": "public"},
        )
        target_group = WalletGroupFactory(owner=self.user)
        ...
        response = self.client.post(
            url,
            {
                "alert_group": str(public_alert_group.id),
                "target_group": str(target_group.id),
            },
            format="json",
        )
    ...
    def test_by_alert_group_action(self):
        ...
        GroupSubscriptionFactory(
            alert_group=alert_group, target_group=target1, owner=self.user
        )
        GroupSubscriptionFactory(
            alert_group=alert_group, target_group=target2, owner=self.user
        )
        ...
        GroupSubscriptionFactory(
            alert_group=other_alert, target_group=other_target, owner=self.user
        )
    ...
    def test_by_target_group_action(self):
        ...
        GroupSubscriptionFactory(
            alert_group=alert1, target_group=target_group, owner=self.user
        )
        GroupSubscriptionFactory(
            alert_group=alert2, target_group=target_group, owner=self.user
        )
