...
class TestNotificationChannelEndpointAPI:
    ...
    def setup(self):
        ...
        self.user = User.objects.create_user(
            email="endpoint_api@example.com", first_name="Endpoint", last_name="API"
        )
    ...
    def test_update_endpoint(self):
        ...
        data = {"label": "Updated Work Email", "enabled": False}

        response = self.client.patch(
            f"/api/notifications/channels/{endpoint.id}/", data, format="json"
        )
...
class TestTeamNotificationChannelEndpointAPI:
    ...
    def setup(self):
        ...
        TeamMember.objects.create(
            team=self.team, user=self.owner, role=TeamMemberRole.OWNER
        )
        TeamMember.objects.create(
            team=self.team, user=self.admin, role=TeamMemberRole.ADMIN
        )
        TeamMember.objects.create(
            team=self.team, user=self.member, role=TeamMemberRole.MEMBER
        )
    ...
    def test_owner_can_create_team_endpoint(self):
        ...
        response = self.client.post(
            "/api/team-notification-endpoints/", data, format="json"
        )
...
class TestTeamMemberNotificationOverrideAPI:
    ...
    def setup(self):
        """Set up test data for each test"""
        self.user = User.objects.create_user(
            email="override_api@example.com", first_name="Override", last_name="API"
        )
    ...
    def test_list_user_overrides(self):
        ...
        override = next(
            (o for o in response.data if o["team_id"] == str(self.team.id)), None
        )
    ...
    def test_get_or_create_override_for_team(self):
        ...
        response = self.client.get(f"/api/team-notification-overrides/{self.team.id}/")
    ...
    def test_update_override(self):
        ...
        response = self.client.patch(
            f"/api/team-notification-overrides/{self.team.id}/", data, format="json"
        )
