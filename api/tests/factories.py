...
class UserFactory(DjangoModelFactory):
    ...
    class Params:
        admin = Trait(is_staff=True, is_superuser=True, username="admin")
        premium = Trait(username=factory.Sequence(lambda n: f"premium_user{n}"))
...
class AlertJobSpecFactory(DjangoModelFactory):
    ...
    status = "active"
    generated_by = "llm"
    generation_model = factory.Faker(
        "random_element", elements=["gpt-4", "gpt-3.5-turbo"]
    )
    ...
    class Params:
        ...
        failed = Trait(
            status="failed", validation_errors={"error": "Invalid query syntax"}
        )
...
class BlockchainNodeFactory(DjangoModelFactory):
    ...
    chain_id = factory.LazyFunction(
        lambda: f"chain-{uuid.uuid4().hex[:12]}"
    )  # Unique chain ID
    ...
    ws_url = factory.LazyAttribute(
        lambda o: f"wss://{o.chain_id}.ws.example.com"
        if o.vm_type in ["EVM", "SVM"]
        else ""
    )
    enabled = True
    is_primary = False
    ...
    success_rate = factory.Faker(
        "pydecimal", left_digits=2, right_digits=1, min_value=90, max_value=99.9
    )
    ...
    class Params:
        ...
        unhealthy = Trait(
            ...
            latency_ms=1500,
            last_health_check=factory.LazyFunction(
